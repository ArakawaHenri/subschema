...
class TestPaperExamples(unittest.TestCase):
    ...
    def test_equiv_multiple_case(self):
        ...
        with self.subTest():
            self.assertFalse(is_equivalent(s1, s2))
        with self.subTest():
            self.assertFalse(is_equivalent(s1, s3))
        with self.subTest():
            self.assertTrue(is_equivalent(s1, s4))
        with self.subTest():
            self.assertTrue(is_equivalent(s2, s3))
        with self.subTest():
            self.assertFalse(is_equivalent(s2, s4))
        with self.subTest():
            self.assertFalse(is_equivalent(s3, s4))
    ...
    def test_bool_enum2(self):
        ...
        s3 = {"allOf": [s1, s2]}
        with self.subTest():
            self.assertTrue(is_subschema(s3, s1))
        with self.subTest():
            self.assertTrue(is_subschema(s3, s2))
        with self.subTest():
            self.assertFalse(is_subschema(s1, s3))
        with self.subTest():
            self.assertTrue(is_equivalent(s2, s3))

        s4 = {"anyOf": [s1, s2]}
        with self.subTest():
            self.assertTrue(is_subschema(s1, s4))
        with self.subTest():
            self.assertTrue(is_subschema(s2, s4))
        with self.subTest():
            self.assertFalse(is_subschema(s4, s2))
        with self.subTest():
            self.assertTrue(is_equivalent(s1, s4))
    ...
    def test_union_of_tuples_distributes_finite_item_anyof(self):
        ...
        with self.subTest("missing tuple branch is a concrete counterexample"):
            self.assertFalse(is_subschema(s1, only_string_head))
...
class TestPaperBigchainDB(unittest.TestCase):
    ...
    def test_transaction_create(self):
        ...
        self.assertTrue(is_equivalent(v1, v2))
    ...
    def test_transaction_create_transfer(self):
        ...
        with self.subTest():
            self.assertFalse(is_subschema(transfer_v1, create_v1))
        with self.subTest():
            self.assertFalse(is_subschema(transfer_v2, create_v2))

        with self.subTest():
            self.assertFalse(is_subschema(create_v1, transfer_v1))
        with self.subTest():
            self.assertFalse(is_subschema(create_v2, transfer_v2))
    ...
    def test_transaction(self):
        ...
        with self.subTest():
            self.assertFalse(is_subschema(v1, v2))

        with self.subTest():
            self.assertFalse(is_subschema(v2, v1))
