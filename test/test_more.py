...
from subschema import Dialect, is_subschema, is_equivalent
...
class TestPaperExamples(unittest.TestCase):
    ...
    def test_simple_list_of_type(self):
        ...
        self.assertTrue(is_equivalent(s1, s2, dialect=Dialect.DRAFT4))
    ...
    def test_equiv_multiple_case(self):
        ...
        with self.subTest():
            self.assertFalse(is_equivalent(s1, s2, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertFalse(is_equivalent(s1, s3, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertTrue(is_equivalent(s1, s4, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertTrue(is_equivalent(s2, s3, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertFalse(is_equivalent(s2, s4, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertFalse(is_equivalent(s3, s4, dialect=Dialect.DRAFT4))
    ...
    def test_bool_enum2(self):
        ...
        s3 = {"allOf": [s1, s2]}
        with self.subTest():
            self.assertTrue(is_subschema(s3, s1, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertTrue(is_subschema(s3, s2, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertFalse(is_subschema(s1, s3, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertTrue(is_equivalent(s2, s3, dialect=Dialect.DRAFT4))

        s4 = {"anyOf": [s1, s2]}
        with self.subTest():
            self.assertTrue(is_subschema(s1, s4, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertTrue(is_subschema(s2, s4, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertFalse(is_subschema(s4, s2, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertTrue(is_equivalent(s1, s4, dialect=Dialect.DRAFT4))
    
    def test_union_of_tuples_distributes_finite_item_anyof(self):
        ...
        only_string_head = {"anyOf": [
            {"type": "array", "items": [t1, t],
                "additionalItems": False, "minItems": 2}
        ]}
        with self.subTest("missing tuple branch is a concrete counterexample"):
            self.assertFalse(is_subschema(s1, only_string_head, dialect=Dialect.DRAFT4))
...
class TestPaperBigchainDB(unittest.TestCase):
    ...
    def test_transaction_create(self):
        ...
        self.assertTrue(is_equivalent(v1, v2, dialect=Dialect.DRAFT4))
    ...
    def test_transaction_create_transfer(self):
        ...
        with self.subTest():
            self.assertFalse(is_subschema(transfer_v1, create_v1, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertFalse(is_subschema(transfer_v2, create_v2, dialect=Dialect.DRAFT4))

        with self.subTest():
            self.assertFalse(is_subschema(create_v1, transfer_v1, dialect=Dialect.DRAFT4))
        with self.subTest():
            self.assertFalse(is_subschema(create_v2, transfer_v2, dialect=Dialect.DRAFT4))
    ...
    def test_transaction(self):
        ...
        with self.subTest():
            self.assertFalse(is_subschema(v1, v2, dialect=Dialect.DRAFT4))

        with self.subTest():
            self.assertFalse(is_subschema(v2, v1, dialect=Dialect.DRAFT4))
    ...
    def test_transaction_vote_transaction_validator_election(self):
        ...
        with self.subTest():
            self.assertTrue(is_subschema(
                transaction_validator_election, transaction_vote, dialect=Dialect.DRAFT4))

        with self.subTest():
            self.assertFalse(is_subschema(transaction_vote,
                                         transaction_validator_election, dialect=Dialect.DRAFT4))
