...
def is_const_subschema(s1, s2):
    return is_subschema(s1, s2, dialect=Dialect.DRAFT7)
...
class TestConst(unittest.TestCase):
    def test_const_equal_num(self) -> None:
        ...
        with self.subTest("LHS < RHS"):
            assert is_const_subschema(s1, s2)
        with self.subTest("LHS > RHS"):
            assert is_const_subschema(s2, s1)
    ...
    def test_const_vs_enum(self) -> None:
        ...
        with self.subTest("LHS < RHS"):
            assert is_const_subschema(s1, s2)
        with self.subTest("LHS > RHS"):
            assert not is_const_subschema(s2, s1)
    ...
    def test_const_vs_wrong_enum(self) -> None:
        ...
        with self.subTest("LHS < RHS"):
            assert not is_const_subschema(s1, s2)
        with self.subTest("LHS > RHS"):
            assert not is_const_subschema(s2, s1)
...
class TestComplexConst(unittest.TestCase):
    def test_array(self) -> None:
        s1 = {"const": []}
        s2 = {"type": "array"}

        with self.subTest():
            self.assertTrue(is_const_subschema(s1, s2))

        with self.subTest():
            self.assertFalse(is_const_subschema(s2, s1))
