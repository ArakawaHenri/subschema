...
class TestNumericSubtype(unittest.TestCase):
    ...
    def test_numeric_singleton_intervals_are_finite_for_applicators(self):
        ...
        with self.subTest("integer singleton in exactly one branch"):
            self.assertTrue(is_subschema({"type": "integer", "minimum": 1, "maximum": 1}, rhs))
        with self.subTest("integer singleton in another exactly one branch"):
            self.assertTrue(is_subschema({"type": "integer", "minimum": 3, "maximum": 3}, rhs))
        with self.subTest("integer singleton in overlapping branches"):
            self.assertFalse(is_subschema({"type": "integer", "minimum": 2, "maximum": 2}, rhs))
        with self.subTest("number singleton interval"):
            self.assertTrue(is_subschema({"type": "number", "minimum": .5, "maximum": .5}, {"enum": [.5]}))

    def test_bounded_numeric_intervals_are_finite_for_applicators(self):
        with self.subTest("integer interval covered by enum"):
            self.assertTrue(is_subschema({"type": "integer", "minimum": 1, "maximum": 3}, {"enum": [1, 2, 3]}))
        with self.subTest("integer interval not covered by enum"):
            self.assertFalse(is_subschema({"type": "integer", "minimum": 1, "maximum": 3}, {"enum": [1, 2, 4]}))
