...
class TestNumberSubtype(unittest.TestCase):
    ...
    def test_number_is_not_subschema_of_fractional_multiple(self):
        s1 = {"type": "number"}
        s2 = {"type": "number", "multipleOf": .5}
        self.assertFalse(is_subschema(s1, s2, dialect=Dialect.DRAFT4))

    def test_open_small_number_interval_is_not_subschema_of_fractional_multiple(self):
        s1 = {
            "type": "number",
            "minimum": 0,
...
class TestNumericSubtype(unittest.TestCase):
    ...
    def test_numeric_singleton_intervals_are_finite_for_applicators(self):
        rhs = {"oneOf": [{"enum": [1, 2]}, {"enum": [2, 3]}]}
        with self.subTest("integer singleton in exactly one branch"):
            self.assertTrue(is_subschema({"type": "integer", "minimum": 1, "maximum": 1}, rhs, dialect=Dialect.DRAFT4))
        with self.subTest("integer singleton in another exactly one branch"):
            self.assertTrue(is_subschema({"type": "integer", "minimum": 3, "maximum": 3}, rhs, dialect=Dialect.DRAFT4))
        with self.subTest("integer singleton in overlapping branches"):
            self.assertFalse(is_subschema({"type": "integer", "minimum": 2, "maximum": 2}, rhs, dialect=Dialect.DRAFT4))
        with self.subTest("number singleton interval"):
            self.assertTrue(is_subschema({"type": "number", "minimum": .5, "maximum": .5}, {"enum": [.5]}, dialect=Dialect.DRAFT4))

    def test_bounded_numeric_intervals_are_finite_for_applicators(self):
        with self.subTest("integer interval covered by enum"):
            self.assertTrue(is_subschema({"type": "integer", "minimum": 1, "maximum": 3}, {"enum": [1, 2, 3]}, dialect=Dialect.DRAFT4))
        with self.subTest("integer interval not covered by enum"):
            self.assertFalse(is_subschema({"type": "integer", "minimum": 1, "maximum": 3}, {"enum": [1, 2, 4]}, dialect=Dialect.DRAFT4))
        with self.subTest("integer interval covered by anyOf finite union"):
            self.assertTrue(
                is_subschema(
                    {"type": "integer", "minimum": 1, "maximum": 3},
                    {"anyOf": [{"enum": [1, 2]}, {"enum": [3]}]},
                    dialect=Dialect.DRAFT4,
                )
            )
        with self.subTest("integer interval covered by disjoint oneOf finite union"):
            self.assertTrue(
                is_subschema(
                    {"type": "integer", "minimum": 1, "maximum": 3},
        ...
        with self.subTest("integer interval not covered by overlapping oneOf finite union"):
            self.assertFalse(
                is_subschema(
                    {"type": "integer", "minimum": 1, "maximum": 3},
        ...
        with self.subTest("fractional multiple interval covered by enum"):
            self.assertTrue(
                is_subschema(
                    {"type": "number", "minimum": 0, "maximum": 1, "multipleOf": .5},
                    {"enum": [0, .5, 1]},
                    dialect=Dialect.DRAFT4,
                )
            )
        with self.subTest("fractional multiple interval not covered by enum"):
            self.assertFalse(
                is_subschema(
                    {"type": "number", "minimum": 0, "maximum": 1, "multipleOf": .5},
                    {"enum": [0, 1]},
    ...
    def test_not1(self):
        ...
        #     self.assertFalse(is_subschema(s1, s1, dialect=Dialect.DRAFT4))
        # with self.subTest('RHS > LHS'):
        #     self.assertTrue(is_subschema(s2, s1, dialect=Dialect.DRAFT4))


class TestCompositeNumericSubtype(unittest.TestCase):
    def test_overlapping_oneof_numeric_branch_uses_non_integer_witness(self):
        lhs = {"oneOf": [{"type": "number"}, {"type": "integer"}]}
        rhs = {"type": "number", "minimum": 0}
    ...
    def test_invalid_schema(self):
        ...
        with self.subTest():
            self.assertRaises(SchemaError, is_subschema, s1, s2)
        with self.subTest():
            self.assertRaises(SchemaError, is_subschema, s2, s1)
