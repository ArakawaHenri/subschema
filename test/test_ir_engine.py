...
class TestProofEngineRouting(unittest.TestCase):
    def test_public_api_routes_through_proof_engine(self):
        public_entrypoints = (public_api.is_subschema, public_api.meet_schemas, public_api.join_schemas)
        public_sources = "\n".join(inspect.getsource(entrypoint) for entrypoint in public_entrypoints)

        self.assertIn("ProofEngine", public_sources)
        self.assertNotIn("should_use_ir_engine", public_sources)
        self.assertNotIn("operand pipeline", public_sources)

    def test_public_api_module_uses_modern_normalization(self):
        api_source = inspect.getsource(public_api)
        self.assertNotIn("json" "ref", api_source)
        self.assertNotIn("normalize_modern_refs", api_source)
        self.assertIn("normalize_boolean_schemas", api_source)

        self.assertEqual(public_api.canonicalize_schema(True, dialect=Dialect.DRAFT6), {})
        self.assertEqual(public_api.canonicalize_schema(False, dialect=Dialect.DRAFT6), {"not": {}})
    ...
    def test_runtime_budget_consumers_use_unified_work_meter(self):
        ...
        self.assertIn("default_search_horizon", inspect.getsource(context_module))
        self.assertIn("allows_expensive_proof", inspect.getsource(context_module))
        ...
        self.assertIn("proof_work_label", inspect.getsource(context_module))

    def test_expensive_proof_gate_uses_typed_policy_and_work_meter(self):
        default_context = ProofContext(Dialect.DRAFT7)
        endeavor_context = ProofContext(Dialect.DRAFT7, ProofOptions(endeavor=True, budgets=ProofBudgets(max_work=0)))

        default_result = default_context.enter_expensive_proof("object_product")
        exhausted_result = endeavor_context.enter_expensive_proof("object_product", units=1)
        ...
        self.assertEqual(default_result.reason, "object product requires endeavor proof")
        self.assertEqual(exhausted_result.status, "resource_exhausted")
        self.assertEqual(exhausted_result.reason, "object product exceeded proof work budget")
    ...
    def test_symbolic_solver_timeout_is_resource_exhausted(self):
        ...
        context = ProofContext(Dialect.DRAFT7, ProofOptions(endeavor=True, budgets=ProofBudgets(timeout_ms=10)))
    ...
    def test_symbolic_solver_unknown_is_unsupported(self):
        ...
        self.assertEqual(proof.reason, "array product solver returned unknown: incomplete")
    ...
    def test_regular_language_operations_are_routed_through_regex_backend(self):
        for module in (difference_module, objects_module, overlaps_module, strings_module):
            source = inspect.getsource(module)
    ...
    def test_difference_formula_lowering_golden_cases(self):
        ...
        self.assertIs(rhs_all_of_formula_node.applicator, rhs_all_of_formula.rhs.root.applicators[0])
        ...
        self.assertIsNone(rhs_all_of_budget.exhausted_reason_for(current_expansions=0, max_work=2))
        self.assertEqual(
            rhs_all_of_budget.exhausted_reason_for(current_expansions=0, max_work=1),
            rhs_all_of_nnf.branch_budget_exhausted_reason,
        )
        self.assertTrue(all(child.polarity == "negative" for child in rhs_all_of_nnf.children))
        ...
        self.assertIs(rhs_any_of_formula_node.applicator, rhs_any_of_formula.rhs.root.applicators[0])
        ...
        self.assertEqual(applicator_branch_expansion_budget(rhs_any_of_nnf).product_count, 2)
        self.assertTrue(all(child.polarity == "negative" for child in rhs_any_of_nnf.children))
        ...
        self.assertIs(rhs_one_of_formula_node.applicator, rhs_one_of_formula.rhs.root.applicators[0])
        ...
        self.assertEqual(unsupported_nnf_product_plan.unsupported_reason, rhs_one_of_nnf.reason)
        ...
        self.assertEqual(one_of_coverage_expansion_budget(rhs_one_of_plan).product_count, 2)
        self.assertEqual(one_of_disjointness_expansion_budget(rhs_one_of_plan).product_count, 1)
        self.assertEqual(
        ...
        self.assertIsInstance(rhs_one_of_overlap_selection, ApplicatorOneOfCoveringSelection)
        self.assertFalse(rhs_one_of_overlap_selection.is_selected)
        self.assertIsInstance(rhs_one_of_overlap_selection.overlap_product, ApplicatorOneOfOverlapProduct)
        self.assertEqual(
        ...
        self.assertIs(lhs_not_wrapper.applicator, lhs_not_formula.lhs.root.applicators[0])
        ...
        self.assertIs(lhs_conditional_formula_node.source, lhs_conditional_formula.lhs.root)
        ...
        self.assertIs(lhs_conditional_formula_node.applicator, lhs_conditional_formula.lhs.root.applicators[0])
        self.assertIs(lhs_conditional_formula_node.condition_node, lhs_conditional_formula.lhs.root.applicators[0].children[0])
        self.assertIs(lhs_conditional_formula_node.then_node, lhs_conditional_formula.lhs.root.applicators[1].children[0])
        self.assertIs(lhs_conditional_formula_node.else_node, lhs_conditional_formula.lhs.root.applicators[2].children[0])
        ...
        self.assertEqual([branch.kind for branch in lhs_conditional_plan.branches], ["if-true", "if-false"])
        self.assertEqual(lhs_conditional_plan.branches[0].condition.polarity, "positive")
        self.assertEqual(lhs_conditional_plan.branches[0].consequence.polarity, "positive")
        self.assertEqual(lhs_conditional_plan.branches[1].condition.polarity, "negative")
        self.assertEqual(lhs_conditional_plan.branches[1].consequence.polarity, "positive")
    ...
    def test_difference_formula_owns_unsupported_diagnostics(self):
        ...
        proof = EmptinessSolver(ProofContext(Dialect.DRAFT202012)).prove_formula_difference_empty(formula)
        self.assertEqual(proof.status, "unsupported")
    ...
    def test_sat_emptiness_solver_lives_in_kernel_package(self):
        ...
        self.assertEqual(formula.occurrences, (formula.positive_lhs, formula.negative_rhs))
        self.assertEqual(ApplicatorBranchPlan.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorBranchProduct.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorConditionalBranch.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorConditionalPlan.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorConditionalProduct.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorFormulaFragment.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorNnfChild.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorNnfBranchProduct.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorNnfFragment.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorOneOfBranchProduct.__module__, "subschema.kernel.applicators")
        self.assertEqual(ApplicatorOneOfCardinalityPlan.__module__, "subschema.kernel.applicators")
        ...
        self.assertEqual(ArrayContainsConstraint.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayContainsDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayContainsItemProof.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayContainsMaxViolationPlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayContainsMinViolationPlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayDuplicateWitnessPlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayItemValueObligation.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayItemValuesDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayLengthDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayUnevaluatedItemObligation.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayUnevaluatedItemsDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ArrayUniquenessDifferencePlan.__module__, "subschema.kernel.difference")
        ...
        self.assertEqual(materialize_array_duplicate_witness_plan.__module__, "subschema.kernel.difference")
        self.assertEqual(materialize_array_witness_plan.__module__, "subschema.kernel.difference")
        self.assertEqual(materialize_array_witness_skeleton.__module__, "subschema.kernel.difference")
        self.assertEqual(materialize_object_key_value_witness_skeleton.__module__, "subschema.kernel.difference")
        self.assertEqual(materialize_object_property_value_witness_skeleton.__module__, "subschema.kernel.difference")
        self.assertEqual(ClosedObjectDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ClosedObjectValueObligation.__module__, "subschema.kernel.difference")
        self.assertEqual(ClosedObjectWitnessSkeleton.__module__, "subschema.kernel.difference")
        self.assertEqual(ClosedObjectWitnessSlot.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectDifferenceModel.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectKeyValueDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectKeyValueObligation.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectKeyValueShape.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectKeyValueWitnessSkeleton.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectKeyValueWitnessSlot.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPropertyCountDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPropertyValueObligation.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPropertyValuesDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPropertyValueWitnessSkeleton.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPropertyValueWitnessSlot.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPropertyNamesDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPropertyNamesRepairSkeleton.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPropertyNamesRepairSlot.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectUnevaluatedPropertiesDifferencePlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectUnevaluatedPropertyObligation.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPresenceProductPlan.__module__, "subschema.kernel.difference")
        self.assertEqual(ObjectPresenceWitnessPlan.__module__, "subschema.kernel.difference")
        self.assertEqual(materialize_closed_object_witness_skeleton.__module__, "subschema.kernel.difference")
        self.assertEqual(materialize_object_property_names_repair_skeleton.__module__, "subschema.kernel.difference")
        ...
        self.assertEqual(EvaluationEffectFormula.__module__, "subschema.kernel.formulas")
        ...
        self.assertEqual(StringLengthConstraint.__module__, "subschema.kernel.constraints")
        self.assertEqual(StringLanguageConstraint.__module__, "subschema.kernel.constraints")
        self.assertEqual(ArrayLengthConstraint.__module__, "subschema.kernel.constraints")
        self.assertEqual(ArrayUniquenessConstraint.__module__, "subschema.kernel.constraints")
        self.assertEqual(ObjectPropertyCountConstraint.__module__, "subschema.kernel.constraints")
        self.assertEqual(ObjectPropertyNamesConstraint.__module__, "subschema.kernel.constraints")
        self.assertEqual(ObjectPropertyValuesConstraint.__module__, "subschema.kernel.constraints")
        self.assertEqual(ObjectClosedPropertiesConstraint.__module__, "subschema.kernel.constraints")
        ...
        self.assertEqual(EvaluationExpressionOrigin.__module__, "subschema.kernel.evaluation")
        self.assertEqual(EvaluationTraceExpression.__module__, "subschema.kernel.evaluation")
        ...
        self.assertEqual(formula.lhs.source.__class__.__module__, "subschema.kernel.references")
        ...
        self.assertIsInstance(array_length_formula.lhs.array_length_lhs_constraint, ArrayLengthConstraint)
        array_uniqueness_formula = DifferenceFormula.from_schemas(
            {"type": "array", "uniqueItems": True},
            {"type": "array"},
            Dialect.DRAFT7,
        )
        self.assertIsInstance(array_uniqueness_formula.lhs.array_uniqueness_lhs_constraint, ArrayUniquenessConstraint)
        object_count_formula = DifferenceFormula.from_schemas(
            {"type": "object", "minProperties": 1},
            {"type": "object"},
            Dialect.DRAFT7,
        )
        self.assertIsInstance(object_count_formula.lhs.object_property_count_constraint, ObjectPropertyCountConstraint)
        object_names_formula = DifferenceFormula.from_schemas(
            {"type": "object", "propertyNames": {"pattern": "^a"}},
            {"type": "object"},
            Dialect.DRAFT7,
        )
        self.assertIsInstance(object_names_formula.lhs.object_property_names_constraint, ObjectPropertyNamesConstraint)
        object_values_formula = DifferenceFormula.from_schemas(
            {"type": "object", "properties": {"a": {"type": "integer"}}},
            {"type": "object"},
            Dialect.DRAFT7,
        )
        self.assertIsInstance(object_values_formula.lhs.object_property_values_constraint, ObjectPropertyValuesConstraint)
        ...
        self.assertLess(rule_names.index("finite-domain-ir"), rule_names.index("static-reference-ir"))
        self.assertLess(rule_names.index("static-reference-ir"), rule_names.index("dynamic-reference-ir"))
        self.assertLess(rule_names.index("dynamic-reference-ir"), rule_names.index("finite-rhs-domain-ir"))
        self.assertLess(rule_names.index("applicator-left-anyof-ir"), rule_names.index("applicator-left-oneof-ir"))
        self.assertLess(rule_names.index("applicator-left-oneof-ir"), rule_names.index("applicator-left-allof-ir"))
        self.assertLess(rule_names.index("applicator-left-allof-ir"), rule_names.index("applicator-right-not-ir"))
        self.assertLess(rule_names.index("applicator-right-not-ir"), rule_names.index("applicator-right-anyof-ir"))
        self.assertLess(rule_names.index("applicator-right-anyof-ir"), rule_names.index("applicator-right-oneof-ir"))
        self.assertLess(rule_names.index("applicator-right-oneof-ir"), rule_names.index("applicator-right-allof-ir"))
        self.assertLess(rule_names.index("applicator-right-allof-ir"), rule_names.index("applicator-conditional-ir"))
        self.assertNotIn("array-difference-ir", rule_names)
        self.assertNotIn("object-difference-ir", rule_names)
        self.assertLess(rule_names.index("array-unevaluated-items-ir"), rule_names.index("array-length-ir"))
        self.assertLess(rule_names.index("array-length-ir"), rule_names.index("array-uniqueness-ir"))
        self.assertLess(rule_names.index("array-uniqueness-ir"), rule_names.index("array-contains-ir"))
        self.assertLess(rule_names.index("array-contains-ir"), rule_names.index("array-item-values-ir"))
        self.assertLess(
            rule_names.index("object-unevaluated-properties-ir"),
            rule_names.index("object-property-count-ir"),
        )
        self.assertLess(rule_names.index("object-property-count-ir"), rule_names.index("object-presence-product-ir"))
        self.assertLess(rule_names.index("object-presence-product-ir"), rule_names.index("object-property-values-ir"))
        self.assertLess(rule_names.index("object-property-values-ir"), rule_names.index("object-key-value-ir"))
        self.assertLess(rule_names.index("object-key-value-ir"), rule_names.index("object-property-names-ir"))
        self.assertLess(rule_names.index("object-property-names-ir"), rule_names.index("object-closed-properties-ir"))
        self.assertEqual(difference_rules()[-1].name, "object-closed-properties-ir")
        self.assertEqual(tuple(rule.spec for rule in difference_rules()), difference_rule_specs())
        expected_domain_specs = {
            "static-reference-ir": ("static $ref", "exact", "validated", "simple_exact", "branch"),
            "dynamic-reference-ir": ("$dynamicRef", "exact", "validated", "simple_exact", "branch"),
            ...
            "array-length-ir": ("array length", "exact", "validated", "simple_exact", "domain"),
            "array-uniqueness-ir": ("uniqueItems", "bounded_witness", "validated", "simple_exact", "domain"),
            "array-contains-ir": ("contains", "bounded_witness", "validated", "simple_exact", "domain"),
            "array-item-values-ir": ("item-value", "bounded_witness", "validated", "simple_exact", "domain"),
            ...
            "object-property-count-ir": ("property-count", "exact", "validated", "simple_exact", "none"),
            ...
            "object-property-values-ir": ("property-value", "bounded_witness", "validated", "simple_exact", "none"),
            "object-key-value-ir": ("key/value", "bounded_witness", "validated", "simple_exact", "domain"),
            "object-property-names-ir": ("propertyNames", "bounded_witness", "validated", "simple_exact", "domain"),
            "object-closed-properties-ir": ("closed object", "bounded_witness", "validated", "simple_exact", "domain"),
        }
        specs_by_name = {spec.name: spec for spec in difference_rule_specs()}
        for name, (fragment, completeness, witness_mode, proof_class, budget_use) in expected_domain_specs.items():
        ...
        for spec in difference_rule_specs():
            ...
            self.assertIn(spec.completeness, {"bounded_witness", "exact", "unsupported_boundary"})
            self.assertIn(spec.witness_mode, {"none", "validated"})
            self.assertIn(spec.proof_class, {"simple_exact", "endeavor_expensive", "unsupported_unreliable"})
            self.assertIn(spec.budget_use, {"branch", "domain", "none"})
        self.assertNotIn("bounded-witness-search", specs_by_name)
        self.assertEqual(
            {spec.name: spec.completeness for spec in difference_rule_specs()}["type-domain-ir"],
            "exact",
        )
        self.assertEqual(
            {spec.name: spec.completeness for spec in difference_rule_specs()}["applicator-left-anyof-ir"],
            "exact",
        )
        self.assertEqual(
            {spec.name: spec.completeness for spec in difference_rule_specs()}["applicator-right-anyof-ir"],
            "bounded_witness",
        )
        self.assertEqual(
            {spec.name: spec.completeness for spec in difference_rule_specs()}["applicator-right-oneof-ir"],
            "exact",
        )
        self.assertEqual(
            {spec.name: spec.budget_use for spec in difference_rule_specs()}["applicator-conditional-ir"],
            "branch",
        )
        self.assertNotIn("candidate", {spec.witness_mode for spec in difference_rule_specs()})
        self.assertEqual(
            {spec.name: spec.budget_use for spec in difference_rule_specs()}["array-contains-ir"],
            "domain",
        )
        self.assertEqual(
            {spec.name: spec.completeness for spec in difference_rule_specs()}["object-property-count-ir"],
            "exact",
        )
        ...
        self.assertIs(applicator_formula_node.applicator, applicator_fragments[0].source)
        ...
        self.assertIs(applicator_plan_index.branch_with_strategy("left-anyof-exact"), applicator_plan_index.plans[0])
        self.assertEqual(applicator_plan_index.branch_with_strategy("left-anyof-exact"), applicator_plans[0])
        ...
        self.assertEqual(applicator_plans[0].children[0].source.schema, {"type": "string"})
        ...
        mixed_lhs_any_of_fragments = applicator_formula_fragments(mixed_lhs_any_of_formula)
        ...
        self.assertEqual(mixed_lhs_any_of_products[0].lhs_schema, {"allOf": [{"type": "string"}, {"maxLength": 3}]})
        self.assertEqual(mixed_lhs_any_of_products[0].base_schema, {"type": "string"})
        self.assertEqual(
            left_branch_resolved_lhs_schema(mixed_lhs_any_of_products[0], {"pattern": "^a"}),
            {"allOf": [{"type": "string"}, {"pattern": "^a"}]},
        )
        self.assertEqual(left_branch_resolved_lhs_schema(mixed_lhs_any_of_products[0], False), False)
        self.assertEqual(left_branch_resolved_lhs_schema(applicator_products[0], {"type": "number"}), {"type": "number"})
        ...
        self.assertEqual(lhs_one_of_products[0].witness_rejected_reason, "SAT left-oneOf branch witness was rejected")
        self.assertEqual(
        ...
        self.assertEqual(lhs_all_of_products[0].witness_rejected_reason, "SAT left-allOf branch witness was rejected")
        mixed_rhs_any_of_formula = DifferenceFormula.from_schemas(
            {"type": "string", "maxLength": 3},
            {"type": "string", "anyOf": [{"maxLength": 5}, {"pattern": "^a"}]},
            Dialect.DRAFT7,
        )
        mixed_rhs_any_of_fragments = applicator_formula_fragments(mixed_rhs_any_of_formula)
        ...
        self.assertIs(mixed_rhs_any_of_formula_node.source, mixed_rhs_any_of_formula.rhs.root)
        ...
        self.assertIs(mixed_rhs_any_of_formula_node.applicator, mixed_rhs_any_of_fragments[0].source)
        self.assertIs(mixed_rhs_any_of_fragments[0].formula_node, mixed_rhs_any_of_formula_node)
        self.assertEqual(
        ...
        self.assertEqual(mixed_rhs_any_of_base.witness_rejected_reason, "SAT right-anyOf base witness was rejected")
        ...
        mixed_rhs_all_of_fragments = applicator_formula_fragments(mixed_rhs_all_of_formula)
        ...
        self.assertIs(mixed_rhs_all_of_formula_node.applicator, mixed_rhs_all_of_fragments[0].source)
        self.assertIs(mixed_rhs_all_of_fragments[0].formula_node, mixed_rhs_all_of_formula_node)
        self.assertEqual(
        ...
        self.assertEqual(mixed_rhs_all_of_base.witness_rejected_reason, "SAT right-allOf base witness was rejected")
        ...
        self.assertEqual(rhs_not_nnf[0].children[0].node.source.schema, {"type": "number"})
        ...
        self.assertIsInstance(rhs_not_product.rhs_string_language_constraint, StringLanguageConstraint)
        self.assertEqual(rhs_not_product.witness_missing_reason, "SAT right-not witness could not be constructed")
        self.assertEqual(rhs_not_product.witness_rejected_reason, "SAT right-not witness was rejected")
        self.assertEqual(
        ...
        rhs_not_complement = right_not_complement_schema(rhs_not_product, rhs_not_product.rhs_schema)
        ...
        self.assertEqual(mixed_rhs_not_plan.formula.base_schema, {"type": "string", "definitions": {"bad": {"const": "a"}}})
        ...
        self.assertIs(mixed_rhs_not_plan.formula.formula_node.applicator, mixed_rhs_not_plan.source)
        ...
        self.assertEqual(mixed_rhs_not_base.witness_rejected_reason, "SAT right-not base witness was rejected")
        ...
        self.assertEqual(
            ...
            {"allOf": [{"const": "b"}, {"type": "string", "definitions": {"bad": {"const": "a"}}}]},
        )
        self.assertEqual(mixed_rhs_not_product.rhs_schema, {"$ref": "#/definitions/bad"})
        self.assertEqual(right_not_resolved_rhs_schema(mixed_rhs_not_product, {"const": "a"}), {"const": "a"})
        self.assertEqual(
            right_not_resolved_rhs_schema(mixed_rhs_not_product, None),
            {"$ref": "#/definitions/bad"},
        )
        mixed_rhs_not_complement = right_not_complement_schema(mixed_rhs_not_product, {"const": "a"})
        self.assertTrue(
            right_not_complement_needs_subproof(
        ...
        self.assertTrue(all(child.polarity == "negative" for child in rhs_any_of_nnf[0].children))
        ...
        self.assertIs(rhs_one_of_formula.negative_rhs.formula.applicator, rhs_one_of_plan.source)
        self.assertEqual(rhs_one_of_plan.strategy, "right-oneof-cardinality-exact")
        rhs_one_of_products = one_of_cardinality_products(
            rhs_one_of_plan,
            lhs_schema=rhs_one_of_formula.lhs.schema,
        )
        self.assertTrue(all(isinstance(product, ApplicatorOneOfBranchProduct) for product in rhs_one_of_products))
        ...
        self.assertEqual(one_of_branch_resolved_schema(rhs_one_of_products[1], None), {"type": "array"})
        ...
        rhs_one_of_overlap_witness = one_of_overlap_witness_plan(rhs_one_of_overlap_product, Dialect.DRAFT7)
        ...
        self.assertIsInstance(rhs_one_of_disjointness_products[0], ApplicatorOneOfDisjointnessProduct)
        ...
        self.assertEqual(rhs_one_of_disjointness_products[0].lhs_schema, {"type": "object"})
        self.assertEqual(rhs_one_of_disjointness_products[0].branch_schema, {"type": "array"})
        self.assertEqual(
        ...
        self.assertEqual(
            one_of_disjointness_resolved_branch_schema(rhs_one_of_disjointness_products[0], None),
            {"type": "array"},
        )
        ...
        self.assertEqual(mixed_rhs_one_of_plan.formula.formula_node.applicator_kind, "oneOf")
        self.assertIs(mixed_rhs_one_of_plan.formula.formula_node.applicator, mixed_rhs_one_of_plan.source)
        ...
        self.assertEqual(mixed_rhs_one_of_base.lhs_schema, {"type": "string", "minLength": 1})
        ...
        self.assertIs(conditional_formula_node.applicator, conditional_formula.rhs.root.applicators[0])
        ...
        self.assertEqual(applicator_branch_expansion_budget(conditional_plans[0]).product_count, 2)
        self.assertEqual(conditional_plans[0].if_child.source.schema, {"type": "string"})
        self.assertEqual(conditional_plans[0].then_child.source.schema, {"minLength": 1})
        self.assertEqual(conditional_plans[0].else_child.source.schema, False)
        self.assertIs(conditional_formula_node.condition_node, conditional_plans[0].if_child)
        self.assertIs(conditional_formula_node.then_node, conditional_plans[0].then_child)
        self.assertIs(conditional_formula_node.else_node, conditional_plans[0].else_child)
        self.assertEqual([branch.kind for branch in conditional_plans[0].branches], ["if-true", "if-false"])
        self.assertTrue(all(branch.proof_class == "exact" for branch in conditional_plans[0].branches))
        self.assertEqual(conditional_plans[0].branches[0].condition.polarity, "positive")
        self.assertEqual(conditional_plans[0].branches[0].consequence.polarity, "negative")
        self.assertEqual(conditional_plans[0].branches[1].condition.polarity, "negative")
        self.assertEqual(conditional_plans[0].branches[1].consequence.polarity, "negative")
        ...
        self.assertEqual(conditional_products[0].covering_schema, {"not": {"type": "string"}})
        self.assertEqual(conditional_products[0].covering_lhs_schema, {"type": "string"})
        ...
        self.assertEqual(conditional_products[1].lhs_schema, {"allOf": [{"type": "string"}, {"not": {"type": "string"}}]})
        ...
        self.assertEqual(conditional_products[1].covering_lhs_schema, {"type": "string"})
        ...
        mixed_lhs_conditional_plan = applicator_difference_plans(mixed_lhs_conditional_formula)[0]
        ...
        self.assertIs(mixed_lhs_guarded.condition_node, mixed_lhs_conditional_plan.if_child)
        self.assertIs(mixed_lhs_guarded.then_node, mixed_lhs_conditional_plan.then_child)
        ...
        mixed_rhs_conditional_plan = applicator_difference_plans(mixed_rhs_conditional_formula)[0]
        ...
        self.assertEqual(mixed_rhs_conditional_base.lhs_schema, {"type": "string", "minLength": 2})
        ...
        self.assertEqual(
            applicators_module._conditional_nodes_from_formula_metadata(mixed_rhs_conditional_formula.negative_rhs),
            (
        ...
        self.assertEqual(applicator_difference_plans(hard_keyword_conditional_formula), ())
        ...
        self.assertIn("positive_lhs", inspect.getsource(formulas_module.DifferenceFormula))
        self.assertIn("negative_rhs", inspect.getsource(formulas_module.DifferenceFormula))
        self.assertIn("_reference_formula", inspect.getsource(formulas_module._positive_formula_for_node))
        self.assertIn("unsupported_diagnostics", inspect.getsource(formulas_module.FormulaOccurrence))
        self.assertIn("formula.unsupported_diagnostics", inspect.getsource(sat_module._semantic_unsupported))
        self.assertFalse(hasattr(sat_module, "_unsupported_diagnostics"))
        self.assertIn("def applicator_plan_set", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIn("applicator_plan_set(self.formula)", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIn("def applicator_plans", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIn("return self.applicator_plan_set.plans", inspect.getsource(sat_module.DifferenceProblem))
        self.assertNotIn("for plan in problem.applicator_plans", inspect.getsource(sat_module))
        ...
        self.assertNotIn("applicator_difference_plans(problem.formula)", inspect.getsource(sat_module))
        self.assertNotIn("applicator_difference_plans(problem.formula.lhs", inspect.getsource(sat_module))
        self.assertIn("FormulaOccurrence", inspect.getsource(applicators_module))
        self.assertIn("_applicator_formula_from_metadata", inspect.getsource(applicators_module))
        ...
        self.assertFalse(hasattr(applicators_module, "_applicator_node_from_formula_metadata"))
        self.assertIn(
            "_applicator_formula_from_metadata(occurrence, kind)",
            inspect.getsource(applicators_module._pure_applicator_formula),
        )
        self.assertNotIn("_applicator_node_from_ir", inspect.getsource(applicators_module._pure_applicator_formula))
        self.assertIn(
            "_conditional_formula_from_metadata(occurrence)",
            inspect.getsource(applicators_module._conditional_applicator_plan_for_occurrence),
        )
        self.assertIn("_conditional_nodes_from_formula_metadata", inspect.getsource(applicators_module))
        self.assertIn("_conditional_nodes_from_not_formula", inspect.getsource(applicators_module))
        self.assertIn("_conditional_formula_from_metadata", inspect.getsource(applicators_module))
        ...
        self.assertNotIn(
            ...
            inspect.getsource(applicators_module._conditional_nodes_from_formula_metadata),
        )
        self.assertNotIn("occurrence.ir", inspect.getsource(applicators_module._conditional_formula_from_metadata))
        ...
        self.assertNotIn("applicator_nnf_fragment(plan.formula)", inspect.getsource(sat_module))
        self.assertNotIn("_prove_rhs_not_difference(problem, plan.children[0])", inspect.getsource(sat_module))
        ...
        self.assertIn("right-oneof-cardinality-exact", inspect.getsource(applicators_module))
        self.assertIn("conditional-guarded-exact", inspect.getsource(applicators_module))
        self.assertIn("applicator_branch_products", inspect.getsource(sat_module._left_branch_products))
        self.assertIn("witness_unsupported_reason", inspect.getsource(applicators_module.ApplicatorBranchProduct))
        self.assertIn("product.witness_missing_reason", inspect.getsource(sat_module._prove_left_any_of_difference))
        self.assertIn("product.witness_rejected_reason", inspect.getsource(sat_module._prove_left_any_of_difference))
        self.assertIn("product.witness_unsupported_reason", inspect.getsource(sat_module._prove_left_one_of_difference))
        ...
        self.assertEqual(left_any_of_branch_proof_choice("proved_false"), "validate_witness")
        self.assertEqual(left_one_of_branch_proof_choice("proved_true"), "continue")
        self.assertEqual(left_one_of_branch_proof_choice("unsupported"), "record_unsupported")
        self.assertEqual(left_one_of_branch_proof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(left_one_of_branch_proof_choice("proved_false"), "validate_witness")
        self.assertEqual(left_all_of_branch_proof_choice("proved_true"), "proved_true")
        self.assertEqual(left_all_of_branch_proof_choice("unsupported"), "record_unsupported")
        self.assertEqual(left_all_of_branch_proof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(left_all_of_branch_proof_choice("proved_false"), "validate_witness")
        self.assertIn("left_any_of_branch_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("left_one_of_branch_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("left_all_of_branch_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("left_any_of_branch_proof_choice", inspect.getsource(sat_module._prove_left_any_of_difference))
        self.assertIn("left_one_of_branch_proof_choice", inspect.getsource(sat_module._prove_left_one_of_difference))
        self.assertIn("left_all_of_branch_proof_choice", inspect.getsource(sat_module._prove_left_all_of_difference))
        self.assertNotIn('proof.status == "proved_true"', inspect.getsource(sat_module._prove_left_any_of_difference))
        self.assertNotIn('proof.status == "unsupported"', inspect.getsource(sat_module._prove_left_one_of_difference))
        self.assertNotIn('proof.status == "resource_exhausted"', inspect.getsource(sat_module._prove_left_all_of_difference))
        ...
        self.assertNotIn("left-oneOf branch counterexample", inspect.getsource(sat_module))
        self.assertNotIn("child.source.schema", inspect.getsource(sat_module._prove_left_any_of_difference))
        self.assertNotIn("child.source.schema", inspect.getsource(sat_module._prove_left_one_of_difference))
        self.assertNotIn("child.source.schema", inspect.getsource(sat_module._prove_left_all_of_difference))
        self.assertIn("left_branch_resolved_lhs_schema", inspect.getsource(applicators_module))
        self.assertIn("left_branch_resolved_lhs_schema", inspect.getsource(sat_module._left_branch_subproof))
        ...
        self.assertIn("conditional_branch_products", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertIn("covering_lhs_schema", inspect.getsource(applicators_module.ApplicatorConditionalProduct))
        self.assertIn("product.covering_lhs_schema", inspect.getsource(sat_module._prove_rhs_conditional_product_empty))
        self.assertIn(
            "is_trivially_empty_difference",
            inspect.getsource(applicators_module.ApplicatorConditionalProduct),
        )
        self.assertIn("product.is_trivially_empty_difference", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn("schema_is_false", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn("schema_is_true", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertIn("witness_missing_reason", inspect.getsource(applicators_module.ApplicatorConditionalProduct))
        self.assertIn("product.witness_missing_reason", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertIn("product.witness_rejected_reason", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn("conditional branch witness", inspect.getsource(sat_module))
        self.assertEqual(conditional_covering_subproof_choice("proved_true"), "proved_true")
        self.assertEqual(conditional_covering_subproof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(conditional_covering_subproof_choice("unsupported"), "continue")
        self.assertEqual(conditional_covering_product_proof_choice("proved_true"), "continue")
        self.assertEqual(conditional_covering_product_proof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(conditional_branch_proof_choice("proved_true"), "continue")
        self.assertEqual(conditional_branch_proof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(conditional_branch_proof_choice("unsupported"), "record_unsupported")
        self.assertEqual(conditional_branch_proof_choice("proved_false"), "validate_witness")
        self.assertEqual(
        ...
        self.assertIn("conditional_covering_subproof_choice", inspect.getsource(applicators_module))
        self.assertIn("conditional_covering_product_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("conditional_branch_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("conditional_final_proof_choice", inspect.getsource(applicators_module))
        self.assertIn(
            "conditional_covering_product_proof_choice",
            inspect.getsource(sat_module._prove_conditional_difference),
        )
        self.assertIn("conditional_branch_proof_choice", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertIn("conditional_final_proof_choice", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertIn(
            "conditional_covering_subproof_choice",
            inspect.getsource(sat_module._prove_rhs_conditional_product_empty),
        )
        self.assertNotIn('empty.status == "proved_true"', inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn('proof.status == "proved_true"', inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn('proof.status == "resource_exhausted"', inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn('proof.status == "unsupported"', inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn('base_proof.status == "proved_true"', inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn(
        ...
        self.assertIn("_prove_applicator_base_difference", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertIn("applicator_base_product", inspect.getsource(sat_module._prove_applicator_base_difference))
        self.assertIn("ApplicatorBaseProduct", inspect.getsource(applicators_module))
        self.assertIn("witness_missing_reason", inspect.getsource(applicators_module.ApplicatorBaseProduct))
        self.assertIn("_validated_applicator_base_false", inspect.getsource(sat_module))
        self.assertIn("product.witness_missing_reason", inspect.getsource(sat_module._validated_applicator_base_false))
        self.assertIn("product.witness_rejected_reason", inspect.getsource(sat_module._validated_applicator_base_false))
        self.assertEqual(applicator_base_pre_branch_choice("proved_false"), "base_false")
        self.assertEqual(applicator_base_pre_branch_choice("proved_true"), "continue")
        self.assertIn("applicator_base_pre_branch_choice", inspect.getsource(applicators_module))
        self.assertIn("applicator_base_pre_branch_choice", inspect.getsource(sat_module._prove_right_not_applicator_difference))
        self.assertIn("applicator_base_pre_branch_choice", inspect.getsource(sat_module._prove_conditional_difference))
        self.assertNotIn('base_proof.status == "proved_false"', inspect.getsource(sat_module))
        self.assertEqual(
        ...
        self.assertEqual(
            right_applicator_base_first_result_choice("resource_exhausted", "unsupported"),
            "base",
        )
        self.assertEqual(
        ...
        self.assertEqual(right_applicator_branch_first_pre_base_choice("resource_exhausted"), "branch")
        self.assertEqual(right_applicator_branch_first_pre_base_choice("proved_false"), "branch")
        self.assertEqual(right_applicator_branch_first_pre_base_choice("proved_true"), "continue")
        self.assertEqual(
        ...
        self.assertIn("right_applicator_base_first_result_choice", inspect.getsource(applicators_module))
        self.assertIn("right_applicator_branch_first_result_choice", inspect.getsource(applicators_module))
        self.assertIn(
            "right_applicator_base_first_result_choice",
        ...
        self.assertIn("branch_budget_exhausted_reason", inspect.getsource(applicators_module.ApplicatorBranchPlan))
        self.assertNotIn("plan.branch_product_count", inspect.getsource(sat_module._prove_left_any_of_difference))
        self.assertNotIn("plan.branch_budget_exhausted_reason", inspect.getsource(sat_module._prove_left_any_of_difference))
        self.assertIn(
            "_applicator_expansion_budget_exhausted",
        ...
        self.assertNotIn("nnf.branch_product_count", inspect.getsource(sat_module._prove_rhs_negative_all_of_difference))
        self.assertNotIn("nnf.branch_budget_exhausted_reason", inspect.getsource(sat_module._prove_rhs_negative_all_of_difference))
        self.assertIn(
        ...
        self.assertIn("witness_missing_reason", inspect.getsource(applicators_module.ApplicatorNnfBranchProduct))
        self.assertIn("product.witness_rejected_reason", inspect.getsource(sat_module._prove_rhs_negative_any_of_difference))
        self.assertIn("product.witness_missing_reason", inspect.getsource(sat_module._prove_rhs_negative_all_of_difference))
        self.assertIn("product.witness_rejected_reason", inspect.getsource(sat_module._prove_rhs_negative_all_of_difference))
        self.assertEqual(right_negative_any_of_branch_proof_choice("proved_true"), "proved_true")
        self.assertEqual(right_negative_any_of_branch_proof_choice("unsupported"), "record_unsupported")
        self.assertEqual(right_negative_any_of_branch_proof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(right_negative_any_of_branch_proof_choice("proved_false"), "validate_witness")
        self.assertEqual(right_negative_all_of_branch_proof_choice("proved_true"), "continue")
        self.assertEqual(right_negative_all_of_branch_proof_choice("unsupported"), "return_proof")
        self.assertEqual(right_negative_all_of_branch_proof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(right_negative_all_of_branch_proof_choice("proved_false"), "validate_witness")
        self.assertIn("right_negative_any_of_branch_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("right_negative_all_of_branch_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("right_negative_any_of_branch_product_plan", inspect.getsource(applicators_module))
        self.assertIn("right_negative_all_of_branch_product_plan", inspect.getsource(applicators_module))
        self.assertIn("ApplicatorNnfBranchProductPlan", inspect.getsource(applicators_module))
        self.assertIn(
            "right_negative_any_of_branch_product_plan",
        ...
        self.assertNotIn('proof.status == "proved_true"', inspect.getsource(sat_module._prove_rhs_negative_any_of_difference))
        self.assertNotIn('proof.status in {"unsupported", "resource_exhausted"}', inspect.getsource(sat_module._prove_rhs_negative_all_of_difference))
        ...
        self.assertIn("product.covering_schema", inspect.getsource(sat_module._prove_rhs_conditional_product_empty))
        self.assertNotIn("plan.side", inspect.getsource(sat_module._prove_rhs_conditional_product_empty))
        self.assertNotIn("plan.polarity", inspect.getsource(sat_module._prove_rhs_conditional_product_empty))
        self.assertNotIn("problem.lhs_schema", inspect.getsource(sat_module._prove_rhs_conditional_product_empty))
        self.assertNotIn("condition.node.source.schema", inspect.getsource(sat_module._prove_rhs_conditional_product_empty))
        self.assertEqual(one_of_coverage_branch_proof_choice("proved_true"), "record_covering")
        self.assertEqual(one_of_coverage_branch_proof_choice("unsupported"), "record_unsupported")
        self.assertEqual(one_of_coverage_branch_proof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(one_of_coverage_branch_proof_choice("proved_false"), "validate_witness")
        self.assertIn("one_of_coverage_branch_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("one_of_cardinality_products", inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertIn("one_of_coverage_branch_proof_choice", inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertIn("one_of_covering_selection", inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertNotIn('proof.status == "proved_true"', inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertNotIn('proof.status == "unsupported"', inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertNotIn("one_of_overlap_product", inspect.getsource(sat_module))
        self.assertNotIn("len(covering_indexes)", inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertIn("one_of_overlap_witness_plan", inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertNotIn("schema_witness_plan", inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertIn("one_of_disjointness_products", inspect.getsource(sat_module._one_of_disjointness_products))
        self.assertIn(
            "one_of_disjointness_complement_schema",
            inspect.getsource(sat_module._prove_rhs_one_of_disjointness_product),
        )
        self.assertEqual(one_of_disjointness_direct_proof_choice("proved_true"), "return_proof")
        self.assertEqual(one_of_disjointness_direct_proof_choice("proved_false"), "return_proof")
        self.assertEqual(one_of_disjointness_direct_proof_choice("resource_exhausted"), "return_proof")
        self.assertEqual(one_of_disjointness_direct_proof_choice("unsupported"), "continue")
        self.assertEqual(one_of_disjointness_proof_choice("proved_true"), "proved_true")
        self.assertEqual(one_of_disjointness_proof_choice("proved_false"), "validate_witness")
        self.assertEqual(one_of_disjointness_proof_choice("unsupported"), "return_proof")
        self.assertEqual(one_of_disjointness_proof_choice("resource_exhausted"), "return_proof")
        self.assertIn("one_of_disjointness_direct_proof_choice", inspect.getsource(applicators_module))
        self.assertIn(
            "one_of_disjointness_direct_proof_choice",
            inspect.getsource(sat_module._prove_rhs_one_of_disjointness_product),
        )
        self.assertIn("one_of_disjointness_proof_choice", inspect.getsource(applicators_module))
        self.assertIn(
            "one_of_disjointness_proof_choice",
            inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference),
        )
        ...
        self.assertNotIn("product.branch_schema", inspect.getsource(sat_module._prove_rhs_one_of_disjointness_product))
        self.assertIn("product.witness_missing_reason", inspect.getsource(sat_module._prove_rhs_one_of_disjointness_product))
        self.assertIn("product.witness_rejected_reason", inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        ...
        self.assertIn("_prove_applicator_base_difference", inspect.getsource(sat_module._prove_right_one_of_applicator_difference))
        self.assertIn("subproof(product.lhs_schema", inspect.getsource(sat_module._prove_rhs_one_of_disjointness_product))
        self.assertNotIn("if product.index == covered_index", inspect.getsource(sat_module._prove_rhs_one_of_cardinality_difference))
        self.assertFalse(hasattr(sat_module, "_rhs_nnf_branch_products"))
        self.assertIn("right_nnf_branch_resolved_rhs_schema", inspect.getsource(sat_module._rhs_nnf_branch_subproof))
        self.assertNotIn("product.rhs_schema", inspect.getsource(sat_module._rhs_nnf_branch_subproof))
        self.assertIn("one_of_branch_resolved_schema", inspect.getsource(sat_module._rhs_one_of_branch_subproof))
        self.assertNotIn("product.branch_schema", inspect.getsource(sat_module._rhs_one_of_branch_subproof))
        self.assertNotIn("child.node.source.schema", inspect.getsource(sat_module._prove_rhs_negative_any_of_difference))
        self.assertNotIn("child.node.source.schema", inspect.getsource(sat_module._prove_rhs_negative_all_of_difference))
        ...
        self.assertIn("ApplicatorNnfFragment", inspect.getsource(sat_module._prove_rhs_negative_any_of_difference))
        self.assertIn("ApplicatorNnfFragment", inspect.getsource(sat_module._prove_rhs_negative_all_of_difference))
        self.assertIn("ApplicatorBranchPlan", inspect.getsource(applicators_module))
        self.assertEqual(right_not_string_overlap_plan.__module__, "subschema.kernel.overlaps")
        self.assertEqual(right_not_string_overlap_plan_from_constraints.__module__, "subschema.kernel.overlaps")
        self.assertEqual(right_not_string_overlap_proof_choice.__module__, "subschema.kernel.overlaps")
        self.assertIn("RightNotStringOverlapPlan", inspect.getsource(overlaps_module))
        self.assertEqual(right_not_string_overlap_proof_choice("proved_true"), "proved_true")
        self.assertEqual(right_not_string_overlap_proof_choice("witness"), "validate_witness")
        self.assertEqual(right_not_string_overlap_proof_choice("unsupported"), "continue")
        self.assertFalse(hasattr(sat_module, "_prove_rhs_not_string_overlap"))
        self.assertNotIn("candidate_string_satisfying_both_schemas", inspect.getsource(sat_module))
        self.assertNotIn("string_language_witness", inspect.getsource(sat_module))
        self.assertNotIn("string_length_fragments_are_disjoint", inspect.getsource(sat_module))
        self.assertIn("right_not_string_overlap_plan_from_constraints", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("right_not_string_overlap_proof_choice", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("lhs_constraint", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("applicator_nnf_schema_product", inspect.getsource(sat_module._rhs_nnf_schema_product))
        self.assertIn("_rhs_not_product_schema", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("product.rhs_string_language_constraint", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("right_not_witness_plan", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("right_not_complement_schema", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("right_not_complement_needs_subproof", inspect.getsource(applicators_module))
        self.assertIn("right_not_complement_needs_subproof", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertEqual(right_not_subproof_choice("proved_true"), "materialize_witness")
        self.assertEqual(right_not_subproof_choice("proved_false"), "continue")
        self.assertEqual(right_not_subproof_choice("resource_exhausted"), "return_resource_exhausted")
        self.assertEqual(right_not_subproof_choice("unsupported"), "continue")
        self.assertEqual(right_not_complement_proof_choice("proved_true"), "proved_true")
        self.assertEqual(right_not_complement_proof_choice("proved_false"), "validate_witness")
        self.assertEqual(right_not_complement_proof_choice("resource_exhausted"), "return_resource_exhausted")
        self.assertEqual(right_not_complement_proof_choice("unsupported"), "continue")
        self.assertIn("right_not_subproof_choice", inspect.getsource(applicators_module))
        self.assertIn("right_not_complement_proof_choice", inspect.getsource(applicators_module))
        self.assertIn("right_not_subproof_choice", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("right_not_complement_proof_choice", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn(
        ...
        self.assertNotIn('proof.status == "proved_true"', inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn('proof.status == "resource_exhausted"', inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn('proof.status == "unsupported"', inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn('complement.status == "proved_true"', inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn('complement.status == "proved_false"', inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn('complement.status == "resource_exhausted"', inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn("schemas_equal", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("right_not_resolved_rhs_schema", inspect.getsource(sat_module._rhs_not_product_schema))
        self.assertNotIn("product.rhs_schema", inspect.getsource(sat_module._rhs_not_product_schema))
        self.assertIn("product.witness_rejected_reason", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("product.complement_witness_missing_reason", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertIn("product.complement_witness_rejected_reason", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn("positive_node.source.schema", inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn('{"not": rhs_schema}', inspect.getsource(sat_module._prove_rhs_not_difference))
        self.assertNotIn("_schema_node_constraint", inspect.getsource(sat_module))
        self.assertIs(composition_module.schemas_are_disjoint, disjointness_module.schemas_are_disjoint)
        self.assertEqual(disjointness_module.schemas_are_disjoint.__module__, "subschema.kernel.disjointness")
        self.assertFalse(hasattr(sat_module, "_prove_schema_disjoint"))
        self.assertNotIn("schema_type_overapproximations_are_disjoint", inspect.getsource(sat_module))
        self.assertNotIn("string_language_fragments_are_disjoint", inspect.getsource(sat_module))
        self.assertFalse(hasattr(sat_module, "WitnessSearchRule"))
        self.assertFalse(hasattr(sat_module, "CandidateGenerator"))
        ...
        self.assertNotIn("first_valid_value_for_schema", inspect.getsource(witnesses_module))
        self.assertNotIn("_typed_witness_candidates", inspect.getsource(witnesses_module))
        self.assertNotIn("CandidateGenerator", inspect.getsource(witnesses_module))
        self.assertIn("finite_values_for_schema", inspect.getsource(ir_module.DomainFacts))
        ...
        self.assertIn("return self.facts.assertion(kind)", inspect.getsource(ir_module.LogicalSchemaIR.assertion))
        self.assertNotIn("for assertion in self.assertions", inspect.getsource(ir_module.LogicalSchemaIR.assertion))
        ...
        self.assertIn("string_length_constraint", inspect.getsource(ir_module.DomainFacts))
        self.assertIn("string_language_constraint", inspect.getsource(ir_module.DomainFacts))
        self.assertIn("array_length_lhs_constraint", inspect.getsource(ir_module.DomainFacts))
        self.assertIn("array_uniqueness_lhs_constraint", inspect.getsource(ir_module.DomainFacts))
        self.assertIn("object_property_count_constraint", inspect.getsource(ir_module.DomainFacts))
        self.assertIn("object_property_names_constraint", inspect.getsource(ir_module.DomainFacts))
        self.assertIn("object_property_values_constraint", inspect.getsource(ir_module.DomainFacts))
        self.assertIn("object_closed_properties_constraint", inspect.getsource(ir_module.DomainFacts))
        self.assertIn("array_length_lhs_constraint", inspect.getsource(difference_module.ArrayDifferenceModel))
        self.assertIn("array_uniqueness_lhs_constraint", inspect.getsource(difference_module.ArrayDifferenceModel))
        self.assertIn("object_property_count_constraint", inspect.getsource(difference_module.ObjectDifferenceModel))
        self.assertIn("object_property_names_constraint", inspect.getsource(difference_module.ObjectDifferenceModel))
        self.assertIn("object_property_values_constraint", inspect.getsource(difference_module.ObjectDifferenceModel))
        self.assertIn("object_closed_properties_constraint", inspect.getsource(difference_module.ObjectDifferenceModel))
        self.assertIn("from_problem", inspect.getsource(difference_module.ArrayDifferenceModel))
        self.assertIn("from_problem", inspect.getsource(difference_module.ObjectDifferenceModel))
        self.assertNotIn("lhs_constraint(", inspect.getsource(difference_module.ArrayDifferenceModel.from_problem))
        self.assertNotIn("rhs_constraint(", inspect.getsource(difference_module.ArrayDifferenceModel.from_problem))
        self.assertNotIn("lhs_constraint(", inspect.getsource(difference_module.ObjectDifferenceModel.from_problem))
        self.assertNotIn("rhs_constraint(", inspect.getsource(difference_module.ObjectDifferenceModel.from_problem))
        self.assertNotIn("unevaluatedProperties", inspect.getsource(ir_module.DomainFacts))
        self.assertNotIn("unevaluatedItems", inspect.getsource(ir_module.DomainFacts))
        self.assertNotIn("finite_values_for_schema", inspect.getsource(sat_module.EmptinessSolver))
        self.assertNotIn("type_shape_for_schema", inspect.getsource(sat_module.EmptinessSolver))
        self.assertNotIn("numeric_shape_for_schema", inspect.getsource(sat_module.EmptinessSolver))
        self.assertNotIn("string_shape_for_schema", inspect.getsource(sat_module.EmptinessSolver))
        self.assertNotIn("array_shape_for_schema", inspect.getsource(sat_module.EmptinessSolver))
        ...
        self.assertIn("def array_model", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIn("ArrayDifferenceModel.from_problem(self)", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIn("def object_model", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIn("ObjectDifferenceModel.from_problem(self)", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIn("def applicator_plan_set", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIn("def applicator_plans", inspect.getsource(sat_module.DifferenceProblem))
        self.assertIsInstance(sat_module.DifferenceProblem.__dict__["array_model"], cached_property)
        self.assertIsInstance(sat_module.DifferenceProblem.__dict__["object_model"], cached_property)
        self.assertIsInstance(sat_module.DifferenceProblem.__dict__["applicator_plan_set"], cached_property)
        self.assertIsInstance(sat_module.DifferenceProblem.__dict__["applicator_plans"], cached_property)
        self.assertIn("problem.array_model", inspect.getsource(sat_module._prove_array_length_difference))
        self.assertIn("finite_constraint", inspect.getsource(sat_module._prove_finite_lhs_difference))
        self.assertIn("lhs_constraint", inspect.getsource(sat_module._prove_finite_lhs_difference))
        self.assertNotIn("formula.lhs.finite_values", inspect.getsource(sat_module._prove_finite_lhs_difference))
        self.assertIn("occurrence_assertion_formula", inspect.getsource(sat_module.DifferenceProblem))
        ...
        self.assertIn("finite_rhs_difference_plan", inspect.getsource(sat_module._prove_finite_rhs_difference))
        self.assertIn("lhs_constraint", inspect.getsource(sat_module._prove_finite_rhs_difference))
        self.assertIn("rhs_constraint", inspect.getsource(sat_module._prove_finite_rhs_difference))
        self.assertNotIn("witness_not_in", inspect.getsource(sat_module._prove_type_difference))
        self.assertNotIn("witness_not_in", inspect.getsource(sat_module._prove_numeric_difference))
        self.assertNotIn("witness_not_in", inspect.getsource(sat_module._prove_string_length_difference))
        self.assertNotIn("witness_not_in", inspect.getsource(sat_module._prove_string_language_difference))
        self.assertIn("type_difference_plan", inspect.getsource(sat_module._prove_type_difference))
        self.assertIn("numeric_difference_plan", inspect.getsource(sat_module._prove_numeric_difference))
        self.assertIn("string_length_difference_plan", inspect.getsource(sat_module._prove_string_length_difference))
        self.assertIn("string_language_difference_plan", inspect.getsource(sat_module._prove_string_language_difference))
        self.assertIn("lhs_constraint", inspect.getsource(sat_module._prove_type_difference))
        self.assertIn("rhs_constraint", inspect.getsource(sat_module._prove_type_difference))
        self.assertIn("lhs_constraint", inspect.getsource(sat_module._prove_numeric_difference))
        self.assertIn("rhs_constraint", inspect.getsource(sat_module._prove_numeric_difference))
        ...
        self.assertNotIn("lhs_length", inspect.getsource(sat_module._prove_array_length_difference))
        self.assertNotIn("rhs_length", inspect.getsource(sat_module._prove_array_length_difference))
        self.assertNotIn("witness_not_in", inspect.getsource(sat_module._prove_array_length_difference))
        self.assertNotIn("_schema_type_is_array_only", inspect.getsource(sat_module))
        self.assertNotIn("contains_empty_min_violation_possible(", inspect.getsource(sat_module._prove_array_contains_difference))
        self.assertNotIn("minimum_contains_matches_guaranteed(", inspect.getsource(sat_module._prove_array_contains_difference))
        self.assertNotIn("contains_min_violation_witness_plan(", inspect.getsource(sat_module._prove_array_contains_difference))
        self.assertNotIn("maximum_contains_matches_possible(", inspect.getsource(sat_module._prove_array_contains_difference))
        self.assertNotIn("_rhs_all_of_evaluated_item_sources", inspect.getsource(sat_module))
        self.assertNotIn("_rhs_evaluated_item_schema_for_index", inspect.getsource(sat_module))
        self.assertNotIn("_first_rhs_unevaluated_item_index_reachable", inspect.getsource(sat_module))
        self.assertNotIn("lhs_length", inspect.getsource(sat_module._prove_array_unevaluated_items_difference))
        self.assertNotIn(
        ...
        self.assertNotIn("_rhs_all_of_evaluated_property_sources", inspect.getsource(sat_module))
        self.assertNotIn("_rhs_evaluated_property_schema_for_name", inspect.getsource(sat_module))
        self.assertNotIn("_rhs_evaluates_property_name", inspect.getsource(sat_module))
        self.assertNotIn("lhs_key_values", inspect.getsource(sat_module._prove_object_unevaluated_properties_difference))
        self.assertNotIn(
            "_object_static_reference_unsupported",
            inspect.getsource(sat_module._prove_object_unevaluated_properties_difference),
        )
        self.assertIn(
            "_lhs_static_reference_unsupported",
            inspect.getsource(sat_module._prove_object_unevaluated_properties_difference),
        )
        self.assertIn("evaluation_trace_for_source", inspect.getsource(difference_module))
        ...
        self.assertIn("evaluated_property_sources", inspect.getsource(difference_module))
        self.assertNotIn("def _all_of_evaluated_item_sources", inspect.getsource(difference_module))
        self.assertNotIn("def _all_of_evaluated_property_sources", inspect.getsource(difference_module))
        self.assertNotIn("lhs_uniqueness", inspect.getsource(sat_module._prove_array_uniqueness_difference))
        self.assertNotIn("rhs_uniqueness", inspect.getsource(sat_module._prove_array_uniqueness_difference))
        self.assertNotIn("uniqueness_duplicate_witness_plan(", inspect.getsource(sat_module._prove_array_uniqueness_difference))
        self.assertNotIn("_is_array_item_values_fragment_schema", inspect.getsource(sat_module))
        self.assertNotIn("has_rhs_item_value_constraints(", inspect.getsource(sat_module._prove_array_item_values_difference))
        self.assertNotIn("item_value_obligations(", inspect.getsource(sat_module._prove_array_item_values_difference))
        self.assertNotIn("object_property_count_shape_for_schema", inspect.getsource(sat_module.EmptinessSolver))
        self.assertIn("problem.object_model", inspect.getsource(sat_module._prove_object_property_count_difference))
        self.assertNotIn("lhs_property_count", inspect.getsource(sat_module._prove_object_property_count_difference))
        self.assertNotIn("rhs_property_count", inspect.getsource(sat_module._prove_object_property_count_difference))
        self.assertNotIn("witness_not_in", inspect.getsource(sat_module._prove_object_property_count_difference))
        ...
        self.assertFalse(hasattr(sat_module, "_object_key_value_mixed_product_supported"))
        ...
        self.assertNotIn("lhs_key_values", inspect.getsource(sat_module._prove_object_key_value_difference))
        self.assertNotIn("rhs_key_values", inspect.getsource(sat_module._prove_object_key_value_difference))
        self.assertNotIn("key_value_product_supported(", inspect.getsource(sat_module._prove_object_key_value_difference))
        self.assertNotIn("key_value_obligations(", inspect.getsource(sat_module._prove_object_key_value_difference))
        self.assertIn(
            "materialize_object_key_value_witness_skeleton",
            inspect.getsource(sat_module._prove_object_key_value_difference),
        )
        self.assertNotIn("lhs_property_values", inspect.getsource(sat_module._prove_object_property_values_difference))
        self.assertNotIn("rhs_property_values", inspect.getsource(sat_module._prove_object_property_values_difference))
        self.assertNotIn("property_value_obligations(", inspect.getsource(sat_module._prove_object_property_values_difference))
        ...
        self.assertNotIn("lhs_closed_properties", inspect.getsource(sat_module._prove_closed_object_properties_difference))
        self.assertNotIn("rhs_closed_properties", inspect.getsource(sat_module._prove_closed_object_properties_difference))
        self.assertNotIn("closed_object_value_obligations(", inspect.getsource(sat_module._prove_closed_object_properties_difference))
        ...
        self.assertNotIn("_rhs_object_property_names_has_value_constraints", inspect.getsource(sat_module))
        self.assertNotIn("lhs_property_names.is_subset_of", inspect.getsource(sat_module))
        ...
        self.assertFalse(hasattr(sat_module, "_first_lhs_unconstrained_index_under_rhs_tail"))
        ...
        self.assertFalse(hasattr(sat_module, "_array_contains_min_violation_witness_from_plan"))
        self.assertFalse(hasattr(sat_module, "_array_contains_max_violation_witness_from_plan"))
        ...
        self.assertIn("materialize_array_witness_skeleton", inspect.getsource(difference_module))
        self.assertIn("materialize_array_witness_plan", inspect.getsource(sat_module._prove_array_contains_difference))
        self.assertFalse(hasattr(sat_module, "_array_length_shape_allows"))
        self.assertNotIn("[None, None]", inspect.getsource(sat_module._prove_array_uniqueness_difference))
        self.assertNotIn("presence_property_sets(", inspect.getsource(sat_module))
        self.assertNotIn("multi_fresh_presence_property_sets(", inspect.getsource(sat_module))
        self.assertNotIn("presence_accepts(", inspect.getsource(sat_module))
        self.assertIn("bounded_ir_proof", inspect.getsource(engine_module.ProofEngine._bounded_ir_proof))
        self.assertIn("EmptinessSolver", inspect.getsource(driver_module.bounded_ir_proof))
        self.assertNotIn("find_counterexample(", inspect.getsource(sat_module))
        self.assertNotIn("find_counterexample(", inspect.getsource(sat_module.EmptinessSolver))
        self.assertNotIn("generate_candidates(", inspect.getsource(composition_module.schemas_are_disjoint))
        self.assertNotIn("CandidateGenerator", inspect.getsource(composition_module.schemas_are_disjoint))
        ...
        rhs_not_witness_constraint_plan = right_not_string_overlap_plan_from_constraints(
            rhs_not_witness_formula.lhs.string_language_constraint,
            rhs_not_witness_nnf.children[0].node.facts.assertion("string-language").value,
    ...
    def test_rule_proof_class_guards_runtime_results(self):
        ...
        endeavor_proof = default_solver.prove_difference_empty({"type": "string"}, {"type": "number"})
        unreliable_proof = unreliable_solver.prove_difference_empty({"type": "string"}, {"type": "number"})
        ...
        self.assertEqual(unreliable_proof.reason, "fake-unreliable is outside the reliable proof fragment")
        ...
        endeavor_allowed = endeavor_solver.prove_difference_empty({"type": "string"}, {"type": "number"})
    ...
    def test_counterexample_certificates_are_verified_before_proved_false(self):
        valid = CounterexampleCertificate(
            ...
            path=("x",),
            children=(CounterexampleCertificate("concrete-witness", "validated concrete child witness"),),
        )
        invalid = CounterexampleCertificate("unknown-certificate", "not a supported certificate")
    ...
    def test_finite_rhs_witnesses_are_constructed_without_candidate_generator(self):
        engine = ProofEngine.for_schemas({"type": "array"}, {"const": []}, dialect=Dialect.DRAFT7)
    ...
    def test_scalar_difference_plans_capture_sat_domain_decisions(self):
        type_formula = DifferenceFormula.from_schemas({"type": "null"}, {"type": "string"}, Dialect.DRAFT7)
        ...
        numeric_formula = DifferenceFormula.from_schemas({"type": "integer"}, {"type": "number"}, Dialect.DRAFT7)
        self.assertIsInstance(numeric_formula.lhs.numeric_constraint, NumericConstraint)
        self.assertEqual(numeric_difference_plan(numeric_formula.lhs, numeric_formula.rhs).status, "proved_true")
        self.assertEqual(
            numeric_difference_plan_from_constraints(
        ...
        with patch.object(
            scalars_module.SymbolicSolver,
            "check_with_work",
            side_effect=AssertionError("constructive numeric witness should not require Z3"),
        ):
        ...
        self.assertIsInstance(length_formula.lhs.string_length_constraint, StringLengthConstraint)
        self.assertEqual(string_length_difference_plan(length_formula.lhs, length_formula.rhs).witness, "aa")
        self.assertEqual(
            string_length_difference_plan_from_constraints(
        ...
        self.assertIsInstance(language_formula.lhs.string_language_constraint, StringLanguageConstraint)
        self.assertEqual(string_language_difference_plan(language_formula.lhs, language_formula.rhs).status, "witness")
        self.assertEqual(
            string_language_difference_plan_from_constraints(
        ...
        finite_formula = DifferenceFormula.from_schemas({"type": "array"}, {"const": []}, Dialect.DRAFT7)
        ...
        self.assertIn("finite_constraint", inspect.getsource(scalars_module.finite_rhs_difference_plan))
        self.assertIn("finite_rhs_difference_plan_from_constraints", inspect.getsource(scalars_module.finite_rhs_difference_plan))
        self.assertIn("type_constraint", inspect.getsource(scalars_module.type_difference_plan))
        self.assertIn("type_difference_plan_from_constraints", inspect.getsource(scalars_module.type_difference_plan))
        self.assertIn("numeric_constraint", inspect.getsource(scalars_module.numeric_difference_plan))
        self.assertIn("numeric_difference_plan_from_constraints", inspect.getsource(scalars_module.numeric_difference_plan))
        self.assertIn("string_length_constraint", inspect.getsource(scalars_module.string_length_difference_plan))
        self.assertIn("string_length_difference_plan_from_constraints", inspect.getsource(scalars_module.string_length_difference_plan))
        self.assertIn("string_language_constraint", inspect.getsource(scalars_module.string_language_difference_plan))
        self.assertIn(
            "string_language_difference_plan_from_constraints",
            inspect.getsource(scalars_module.string_language_difference_plan),
        )
        self.assertNotIn(".type_shape", inspect.getsource(scalars_module.type_difference_plan))
        self.assertNotIn(".numeric_shape", inspect.getsource(scalars_module.numeric_difference_plan))
        self.assertNotIn(".string_length_shape", inspect.getsource(scalars_module.string_length_difference_plan))
        self.assertNotIn(".string_language_shape", inspect.getsource(scalars_module.string_language_difference_plan))

    def test_schema_inhabitant_witness_helpers_live_outside_sat(self):
        self.assertEqual(build_schema_witness.__module__, "subschema.kernel.witnesses")
        self.assertEqual(finite_projection_witness.__module__, "subschema.kernel.witnesses")
        ...
        self.assertEqual(finite_projection_witness({"const": None}, Dialect.DRAFT4).status, "unsupported")
        self.assertEqual(finite_projection_witness({"type": "string"}, Dialect.DRAFT7).status, "unsupported")

    def test_domain_fragments_reject_boolean_integer_keywords(self):
        self.assertIsNone(strings_module.string_shape_for_schema({"type": "string", "minLength": True}))
        self.assertIsNone(strings_module.string_shape_for_schema({"type": "string", "maxLength": False}))
        self.assertIsNone(objects_module.object_property_count_shape_for_schema({"minProperties": True}))
        self.assertIsNone(objects_module.object_property_count_shape_for_schema({"maxProperties": False}))
    ...
    def test_composite_difference_models_capture_object_and_array_search_space(self):
        ...
        array_formula = DifferenceFormula.from_schemas(lhs_array, rhs_array, Dialect.DRAFT202012)
        array_problem = DifferenceProblem(array_formula, ProofEngine(Dialect.DRAFT202012).context)
        array_model = ArrayDifferenceModel.from_problem(array_problem)

        self.assertEqual([(slot.index, slot.source) for slot in array_model.lhs_slots], [(0, "prefixItems")])
        ...
        tail_formula = DifferenceFormula.from_schemas(tail_lhs, tail_rhs, Dialect.DRAFT202012)
        ...
        length_problem = DifferenceProblem(length_formula, ProofEngine(Dialect.DRAFT202012).context)
        ...
        self.assertEqual(
            materialize_array_witness_skeleton(length_plan.witness_skeleton, Dialect.DRAFT202012),
            [None, None, None],
        )
        with patch.object(
        ...
        self.assertTrue(all(isinstance(item, ArrayContainsItemProof) for item in contains_min_plan.item_proofs))
        ...
        self.assertTrue(
            all(isinstance(override, ArrayWitnessOverride) for override in contains_min_witness_plan.overrides)
        )
        ...
        unevaluated_witness_plan = unevaluated_witness_model.unevaluated_items_difference_plan()
        self.assertIsInstance(unevaluated_witness_plan, ArrayUnevaluatedItemsDifferencePlan)
        self.assertEqual(unevaluated_witness_plan.status, "witness")
        self.assertIsInstance(unevaluated_witness_plan.witness_skeleton, ArrayWitnessSkeleton)
        ...
        unevaluated_obligation_plan = unevaluated_obligation_model.unevaluated_items_difference_plan()
        ...
        self.assertTrue(all(isinstance(slot, ArrayWitnessSlot) for slot in array_skeleton.slots))
        self.assertEqual([(slot.index, slot.schema) for slot in array_skeleton.slots], [
            (0, {"type": "integer"}),
            (1, {"type": "string"}),
            (2, {"type": "string"}),
        ])
        materialized_array = materialize_array_witness_skeleton(array_skeleton, Dialect.DRAFT202012)
        ...
        self.assertIsInstance(tail_model.first_lhs_array_witness_skeleton(), ArrayWitnessSkeleton)
        self.assertIsInstance(tail_model.array_witness_skeleton_reaching(2), ArrayWitnessSkeleton)
        self.assertIsInstance(tail_model.array_witness_skeleton_for_length_witness([None, None, None]), ArrayWitnessSkeleton)
        ...
        self.assertEqual((duplicate_plan.first_index, duplicate_plan.second_index), (1, 2))
        self.assertEqual(
        ...
        self.assertIsInstance(tail_model.rhs_closed_tail_violation_skeleton(), ArrayWitnessSkeleton)
        self.assertTrue(all(isinstance(obligation, ArrayItemValueObligation) for obligation in array_obligations))
        self.assertEqual(
        ...
        item_values_plan = item_values_model.item_values_difference_plan(Dialect.DRAFT202012)
        self.assertEqual(item_values_plan.status, "obligations")
        self.assertTrue(all(isinstance(obligation, ArrayItemValueObligation) for obligation in item_values_plan.obligations))
        ...
        object_formula = DifferenceFormula.from_schemas(lhs_object, rhs_object, Dialect.DRAFT202012)
        object_problem = DifferenceProblem(object_formula, ProofEngine(Dialect.DRAFT202012).context)
        ...
        self.assertNotIn(object_model.universe.fresh.representative, object_model.universe.explicit_names)
        self.assertIsInstance(object_model.lhs_key_values, ObjectKeyValueShape)
        self.assertEqual(object_model.lhs_key_values.properties["alpha"], {"type": "integer"})
        ...
        self.assertTrue(all(isinstance(slot, ObjectKeyValueWitnessSlot) for slot in object_skeleton.slots))
        self.assertEqual([(slot.name, slot.schema) for slot in object_skeleton.slots], [
            ("alpha", {"type": "integer"}),
        ])
        materialized_object = materialize_object_key_value_witness_skeleton(object_skeleton, Dialect.DRAFT202012)
        ...
        count_problem = DifferenceProblem(count_formula, ProofEngine(Dialect.DRAFT202012).context)
        ...
        unevaluated_property_witness_plan = unevaluated_property_witness_model.unevaluated_properties_difference_plan()
        self.assertIsInstance(unevaluated_property_witness_plan, ObjectUnevaluatedPropertiesDifferencePlan)
        ...
        unevaluated_property_obligation_plan = (
            unevaluated_property_obligation_model.unevaluated_properties_difference_plan()
        )
        ...
        self.assertTrue(kv_model.rhs_key_values.allows_key(kv_model.universe.fresh.representative))
        obligations = kv_model.key_value_obligations(budget=12)
        self.assertTrue(obligations)
        self.assertTrue(all(isinstance(obligation, ObjectKeyValueObligation) for obligation in obligations))
        self.assertTrue(any(obligation.name.startswith("x") for obligation in obligations))
        ...
        self.assertIsInstance(kv_witness_plan.witness_skeleton, ObjectKeyValueWitnessSkeleton)
        ...
        presence_formula = DifferenceFormula.from_schemas(presence_lhs, presence_rhs, Dialect.DRAFT202012)
        presence_problem = DifferenceProblem(presence_formula, ProofEngine(Dialect.DRAFT202012).context)
        ...
        self.assertTrue(all(isinstance(plan, ObjectPresenceWitnessPlan) for plan in presence_plan.witness_plans))
        self.assertTrue(any(plan.present == frozenset({"alpha"}) for plan in presence_plan.witness_plans))
        ...
        value_formula = DifferenceFormula.from_schemas(value_lhs, value_rhs, Dialect.DRAFT202012)
        value_problem = DifferenceProblem(value_formula, ProofEngine(Dialect.DRAFT202012).context)
        ...
        self.assertTrue(all(isinstance(obligation, ObjectPropertyValueObligation) for obligation in value_obligations))
        self.assertEqual(
            [(obligation.name, obligation.lhs_schema, obligation.rhs_schema) for obligation in value_obligations],
        ...
        self.assertTrue(all(isinstance(slot, ObjectPropertyValueWitnessSlot) for slot in value_skeleton.slots))
        self.assertEqual(
        ...
        materialized_value_override = materialize_object_property_value_witness_skeleton(
            value_skeleton,
            Dialect.DRAFT202012,
            override=("foo", 7),
        )
        ...
        self.assertIsInstance(value_witness_plan.witness_skeleton, ObjectPropertyValueWitnessSkeleton)
        ...
        closed_formula = DifferenceFormula.from_schemas(closed_lhs, closed_rhs, Dialect.DRAFT202012)
        closed_problem = DifferenceProblem(closed_formula, ProofEngine(Dialect.DRAFT202012).context)
        ...
        self.assertTrue(all(isinstance(obligation, ClosedObjectValueObligation) for obligation in closed_obligations))
        self.assertEqual(
            [(obligation.name, obligation.lhs_schema, obligation.rhs_schema) for obligation in closed_obligations],
            [("foo", {"type": "integer"}, {"type": "number"})],
        )
        ...
        self.assertTrue(all(isinstance(slot, ClosedObjectWitnessSlot) for slot in closed_skeleton.slots))
        self.assertEqual(
            [(slot.name, slot.schema) for slot in closed_skeleton.slots],
            [("foo", {"type": "integer"})],
        )
        materialized_closed = materialize_closed_object_witness_skeleton(closed_skeleton, Dialect.DRAFT202012)
        ...
        self.assertTrue(
            all(isinstance(obligation, ClosedObjectValueObligation) for obligation in closed_obligation_plan.obligations)
        )
        ...
        self.assertTrue(all(isinstance(slot, ObjectPropertyNamesRepairSlot) for slot in repair_skeleton.slots))
        self.assertEqual(
            [(slot.name, slot.schema, slot.original_value) for slot in repair_skeleton.slots],
            [("a", {"type": "integer"}, None)],
        )
        ...
        self.assertIsInstance(names_plan.repair_skeleton, ObjectPropertyNamesRepairSkeleton)
        ...
        names_true_problem = DifferenceProblem(names_true_formula, ProofEngine(Dialect.DRAFT202012).context)
        ...
        self.assertEqual(names_true_model.property_names_difference_plan().status, "proved_true")

    def test_array_difference_rule_runs_before_array_exact_tactics_and_generic_search_path(self):
    ...
    def test_static_reference_rule_proves_root_lhs_ref_with_modern_kernel_or_generic_search_path(self):
        lhs = {
            "definitions": {"name": {"type": "string"}},
    ...
    def test_static_reference_rule_validates_rhs_ref_counterexample_with_modern_kernel(self):
        lhs = {"const": -1}
        rhs = {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
    ...
    def test_static_reference_rule_follows_acyclic_pure_ref_chains_with_modern_kernel(self):
        lhs = {
            "definitions": {
        ...
        resolution = references_module.root_static_reference_resolution(formula.lhs, side="lhs")
    ...
    def test_static_reference_chain_reports_recursive_boundary_in_reference_layer(self):
        ...
        formula = DifferenceFormula.from_schemas(lhs, {"type": "string"}, Dialect.DRAFT7)
    ...
    def test_static_reference_dialect_transition_can_still_be_refuted_by_validated_witness(self):
        lhs = True
        rhs = {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "$defs": {
    ...
    def test_static_reference_rule_reports_nested_reference_diagnostic(self):
        ...
        engine = ProofEngine.for_schemas(lhs, {"type": "string"}, dialect=Dialect.DRAFT7)
        ...
        self.assertEqual(proof.diagnostics[0].category, "static-reference")
        self.assertEqual(proof.diagnostics[0].path, ("definitions", "alias", "allOf", "0", "$ref"))
    ...
    def test_left_anyof_with_sibling_base_proves_branch_products_with_modern_kernel(self):
    ...
    def test_left_oneof_with_sibling_base_proves_branch_products_with_modern_kernel(self):
    ...
    def test_left_anyof_sibling_base_counterexample_is_validated_with_modern_kernel(self):
    ...
    def test_right_applicator_branches_resolve_static_ref_children_with_modern_kernel(self):
        lhs = {"type": "string"}
    ...
    def test_right_anyof_with_sibling_base_proves_formula_product_with_modern_kernel(self):
    ...
    def test_right_anyof_sibling_base_counterexample_is_validated_with_modern_kernel(self):
        lhs = {"type": "integer"}
    ...
    def test_right_not_with_sibling_base_resolves_static_ref_child_with_modern_kernel(self):
        lhs = {"const": "b"}
        rhs = {
            "type": "string",
            "definitions": {"bad": {"const": "a"}},
    ...
    def test_right_not_sibling_base_static_ref_overlap_is_validated_with_modern_kernel(self):
        lhs = {"const": "a"}
        rhs = {
            "type": "string",
            "definitions": {"bad": {"const": "a"}},
    ...
    def test_right_not_sibling_base_counterexample_is_validated_with_modern_kernel(self):
        lhs = {"type": "integer"}
    ...
    def test_right_oneof_with_sibling_base_proves_cardinality_products_with_modern_kernel(self):
    ...
    def test_right_oneof_sibling_base_counterexample_is_validated_with_modern_kernel(self):
        lhs = {"type": "integer"}
    ...
    def test_right_allof_with_sibling_base_resolves_static_ref_child_with_modern_kernel(self):
        lhs = {"type": "string"}
        rhs = {
            "type": "string",
            "definitions": {"name": {"type": "string"}},
    ...
    def test_right_allof_sibling_base_counterexample_is_validated_with_modern_kernel(self):
        lhs = {"type": "integer"}
        rhs = {
            "type": "string",
    ...
    def test_left_conditional_with_sibling_base_proves_guarded_products_with_modern_kernel(self):
    ...
    def test_right_conditional_with_sibling_base_proves_base_and_guarded_products_with_modern_kernel(self):
    ...
    def test_right_conditional_sibling_base_counterexample_is_validated_with_modern_kernel(self):
        lhs = {"type": "integer"}
    ...
    def test_array_contains_structural_max_uses_prefix_and_tail_before_array_contains_tactic(self):
        lhs = {
            "type": "array",
            "prefixItems": [{"type": "integer"}],
            "items": {"type": "string"},
    ...
    def test_right_not_array_items_uses_constructive_intersection_witness(self):
        ...
        self.assertTrue(all(isinstance(value, int) and not isinstance(value, bool) for value in proof.witness))
    ...
    def test_right_not_array_contains_uses_constructive_intersection_witness(self):
        ...
        self.assertEqual(proof.status, "proved_false")
        self.assertIsInstance(proof.witness, list)
        self.assertTrue(any(isinstance(value, int) and not isinstance(value, bool) for value in proof.witness))
    ...
    def test_right_not_array_contains_max_uses_constructive_intersection_witness(self):
        cases = (
            (
                ...
                {"not": {"type": "array", "contains": {"type": "integer"}, "minContains": 1, "maxContains": 1}},
            ),
            (
                {"type": "array", "contains": {"type": "integer"}, "minContains": 1},
                {"not": {"type": "array", "contains": {"const": 1}, "minContains": 1, "maxContains": 1}},
            ),
            (
                {"type": "array", "contains": {"const": 1}, "minContains": 1, "maxContains": 1},
                {"not": {"type": "array", "contains": {"type": "integer"}, "minContains": 1, "maxContains": 1}},
            ),
        )
    ...
    def test_right_not_object_uses_constructive_intersection_witness(self):
        cases = (
            (
                {"type": "object"},
                {"not": {"type": "object", "required": ["a"], "properties": {"a": {"type": "integer"}}}},
            ),
            (
                {"type": "object", "minProperties": 1},
                {"not": {"type": "object", "propertyNames": {"pattern": "^a"}, "minProperties": 1}},
            ),
            (
                {"type": "object", "propertyNames": {"pattern": "^a"}, "minProperties": 1},
                {"not": {"type": "object", "required": ["a"], "properties": {"a": {"const": 1}}}},
            ),
        )
        for lhs, rhs in cases:
            with self.subTest(lhs=lhs, rhs=rhs):
    ...
    def test_object_required_property_value_uses_constructive_witness_when_key_is_required(self):
    ...
    def test_array_length_does_not_mask_rhs_unique_items(self):
        ...
        self.assertEqual(proof.status, "proved_false")
        self.assertEqual(len(proof.witness), 2)
        self.assertEqual(json_semantic_key(proof.witness[0]), json_semantic_key(proof.witness[1]))

    def test_contains_constraint_populates_closed_tail_witness(self):
        lhs = {"type": "array", "contains": {"const": 1}, "minContains": 1, "maxContains": 1}
    ...
    def test_unique_array_constructs_distinct_contains_type_min_violation(self):
        ...
        self.assertEqual(proof.status, "proved_false")
        self.assertEqual(len(proof.witness), 2)
        self.assertNotEqual(json_semantic_key(proof.witness[0]), json_semantic_key(proof.witness[1]))
        integer_count = sum(isinstance(value, int) and not isinstance(value, bool) for value in proof.witness)
    ...
    def test_contains_lhs_constructs_duplicate_against_unique_items(self):
        ...
        self.assertLess(len({json_semantic_key(value) for value in proof.witness}), len(proof.witness))
    ...
    def test_contains_lhs_constructs_item_value_witness(self):
        ...
        self.assertEqual(proof.status, "proved_false")
        self.assertEqual(proof.witness.count(1), 1)
        self.assertTrue(any(not isinstance(value, int) or isinstance(value, bool) for value in proof.witness))

    def test_contains_lhs_override_counts_for_rhs_item_not_schema(self):
        lhs = {"type": "array", "contains": {"type": "integer"}, "minContains": 1, "maxContains": 1}
    ...
    def test_array_difference_does_not_treat_rhs_non_array_as_unique_items_success(self):
    ...
    def test_object_difference_rule_runs_before_object_exact_tactics_and_generic_search_path(self):
    ...
    def test_dependent_required_constructs_rhs_property_value_witness(self):
        ...
        self.assertFalse(isinstance(proof.witness["a"], int) and not isinstance(proof.witness["a"], bool))
    ...
    def test_dependent_required_constructs_rhs_not_property_value_witness(self):
        ...
        self.assertIn("a", proof.witness)
        self.assertIn("b", proof.witness)
        self.assertTrue(isinstance(proof.witness["a"], int) and not isinstance(proof.witness["a"], bool))
    ...
    def test_object_presence_product_constructs_multi_fresh_max_properties_witness(self):
    ...
    def test_object_property_values_required_witness_runs_before_property_values_tactic(self):
        lhs = {"type": "object", "properties": {"age": {"type": "integer"}}}
        rhs = {
            "type": "object",
            "properties": {"age": {"type": "integer"}},
    ...
    def test_object_additional_properties_value_rule_runs_before_object_exact_tactics(self):
    ...
    def test_object_property_names_key_value_rule_runs_before_object_exact_tactics(self):
        lhs = {
            "type": "object",
            "propertyNames": {"pattern": "^a"},
    ...
    def test_object_explicit_property_pattern_product_runs_before_object_exact_tactics(self):
        lhs = {
            "type": "object",
    ...
    def test_object_explicit_property_pattern_product_constructs_witness(self):
        ...
        self.assertEqual(proof.status, "proved_false")
        self.assertTrue(any(name.startswith("e") and name != "email" for name in proof.witness))
    ...
    def test_ir_compiles_evaluation_frontier_separately_from_domain_facts(self):
        ...
        self.assertEqual(
            [(source.kind, source.index, source.start_index) for source in compiled.evaluation.item_sources],
            [
                ("prefixItems", 0, None),
                ("items", None, 1),
                ("contains", None, None),
        ...
        self.assertIsInstance(child.evaluation.property_sources[0], EvaluatedPropertySource)
    ...
    def test_ir_collects_nested_unsupported_evaluation_reasons(self):
        ...
        self.assertEqual(compiled.unsupported[0].path, ("allOf", "0", "unevaluatedProperties"))
        self.assertEqual(compiled.unsupported[0].pointer, "#/allOf/0/unevaluatedProperties")
    ...
    def test_ir_source_nodes_preserve_nested_json_pointer_context(self):
        ...
        self.assertEqual(conditional_child.applicators[0].children[0].source.pointer, ("allOf", "1", "if"))
        self.assertEqual(conditional_child.applicators[1].children[0].source.pointer, ("allOf", "1", "then"))
    ...
    def test_validation_unsupported_result_preserves_original_exception_and_diagnostic(self):
        schema = {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "$vocabulary": {
    ...
    def test_acyclic_dynamic_ref_is_no_longer_semantic_unsupported(self):
        ...
        engine = ProofEngine.for_schemas({"type": "string"}, dynamic_ref, dialect=Dialect.DRAFT202012)
    ...
    def test_reference_graph_and_ref_normalization_live_in_kernel_package(self):
        ...
        normalized = references_module.normalize_modern_refs(schema, Dialect.DRAFT202012)
        graph = references_module.ResourceGraph.build(normalized, dialect=Dialect.DRAFT202012)
        ...
        self.assertEqual(graph.resolve_ref("#/$defs/positive"), schema["$defs"]["positive"])
        identified_schema = {
            "$id": "https://example.com/root",
            "$defs": {"name": {"type": "string"}},
            "$ref": "#/$defs/name",
        }
        identified_graph = references_module.ResourceGraph.build(identified_schema, dialect=Dialect.DRAFT7)
        resolution = identified_graph.resolve_ref_info("#/$defs/name")
        self.assertIsInstance(resolution, references_module.ReferenceResolution)
        self.assertEqual(identified_graph.to_ir().resource_uri, "https://example.com/root")
        ...
        dialect_graph = references_module.ResourceGraph.build(dialect_schema, dialect=Dialect.DRAFT7)
        ...
        draft4_id_graph = references_module.ResourceGraph.build(draft4_id_schema, dialect=Dialect.DRAFT4)
        draft4_resolution = draft4_id_graph.resolve_ref_info("number", base_uri="https://example.com/root")
        ...
        compound_graph = references_module.ResourceGraph.build(compound_schema, dialect=Dialect.DRAFT202012)
        self.assertEqual(sorted(compound_graph.resources), ["https://example.com/child", "https://example.com/root"])
        self.assertEqual(compound_graph.resources["https://example.com/child"].pointer, ("$defs", "child"))
        child_resolution = compound_graph.resolve_ref_info("child", base_uri="https://example.com/root")
        ...
        anchor_resolution = compound_graph.resolve_ref_info("#name", base_uri="https://example.com/child")
    ...
    def test_ir_source_nodes_preserve_resource_local_and_document_pointers(self):
        ...
        compiled = SchemaIRCompiler(Dialect.DRAFT202012).compile(schema)
        child_resource = compiled.graph.schema_ir_for_pointer(("$defs", "child"), schema["$defs"]["child"])
        nested_child = SchemaIRCompiler(Dialect.DRAFT202012).compile_graph(compiled.graph).root.applicators[0].children[0]
    ...
    def test_reference_resolution_records_embedded_source_and_target_provenance(self):
        ...
        graph = references_module.ResourceGraph.build(schema, dialect=Dialect.DRAFT202012)
        ...
        self.assertEqual(pointer_resolution.source_resource_uri, "https://example.com/root/child/")
        self.assertEqual(pointer_resolution.source_pointer, ("$defs", "child", "allOf", "0"))
        self.assertEqual(pointer_resolution.source_resource_pointer, ("allOf", "0"))
        self.assertEqual(pointer_resolution.resource_uri, "https://example.com/root/child/")
        self.assertEqual(pointer_resolution.pointer, ("$defs", "escaped/name~token"))
        self.assertEqual(pointer_resolution.document_pointer, ("$defs", "child", "$defs", "escaped/name~token"))
        ...
        self.assertEqual(anchor_resolution.document_pointer, pointer_resolution.document_pointer)
    ...
    def test_dynamic_scope_resolves_nearest_dynamic_anchor(self):
        ...
        resolution = graph.resolve_dynamic_ref_info("#node", root_frame, dynamic_scope=scope)
    ...
    def test_unregistered_external_static_ref_stays_unsupported_with_modern_kernel(self):
        lhs = {
            "$id": "https://example.com/root",
    ...
    def test_schema_position_normalization_lives_in_kernel_package(self):
        ...
        self.assertEqual(normalization_module.normalize_boolean_schemas(schema), expected)
...
class TestMeetJoinProjection(unittest.TestCase):
    ...
    def test_top_bottom_meet_and_join_are_projected_without_removed_checker(self):
        if True:
            ...
            self.assertIs(meet_schemas(False, {"type": "string"}, dialect=Dialect.DRAFT6), False)
...
class TestFiniteDomainProof(unittest.TestCase):
    ...
    def test_right_one_of_with_overlapping_top_and_double_negation_is_not_proved_true(self):
...
class TestApplicatorCompositionProof(unittest.TestCase):
    ...
    def test_lhs_conditional_guarded_products_are_proved_without_removed_checker(self):
        ...
        if True:
            ...
            self.assertFalse(is_subschema({"if": {"type": "string"}, "then": {"type": "integer"}}, rhs, dialect=Dialect.DRAFT7))
...
class TestObjectPresenceDomainProof(unittest.TestCase):
    ...
    def test_presence_product_does_not_collapse_fresh_keyspace_for_overlapping_one_of(self):
    ...
    def test_presence_product_does_not_treat_one_of_as_union_for_value_constraints(self):
        lhs = {"oneOf": [True, {"type": "object", "properties": {"a": {"type": "integer"}}}]}
    ...
    def test_array_valued_dependencies_are_proved_without_removed_checker(self):
        ...
        if True:
            ...
            self.assertTrue(is_subschema(dependency, explicit_required))
            self.assertTrue(is_subschema(explicit_required, dependency))
    ...
    def test_dependent_schemas_all_of_merge_is_proved_without_removed_checker(self):
        ...
        expected = {
            "type": "object",
            "dependentSchemas": {"credit_card": {"required": ["billing_address", "zip"]}},
        }
...
class TestObjectPropertyValuesDomainProof(unittest.TestCase):
    ...
    def test_open_property_value_all_of_constraints_are_combined(self):
        ...
        rhs = {"type": "object", "properties": {"age": {"type": "number"}, "name": {"type": "string"}}}
...
class TestIREngineHardFeatures(unittest.TestCase):
    ...
    def test_evaluation_expression_records_reference_and_branch_provenance(self):
        ...
        static_ref_origin = next(origin for origin in expression.origins if origin.kind == "static-ref")
    ...
    def test_evaluation_expression_cache_reuses_branch_subproofs(self):
        ...
        with patch.object(engine.context, "subproof", wraps=engine.context.subproof) as subproof:
    ...
    def test_unevaluated_items_counts_contains_matches(self):
        ...
        with self.subTest("contains match is evaluated"):
            self.assertTrue(is_subschema({"const": [1]}, schema, dialect=Dialect.DRAFT202012))
