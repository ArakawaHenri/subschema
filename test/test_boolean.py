...
class TestNotBoolean(unittest.TestCase):
    ...
    def test_not_empty_array_has_non_array_type_counterexamples(self):
        s1 = {'not': {'type': 'array', 'maxItems': 0}}

        with self.subTest('not empty array is not only strings'):
            self.assertFalse(is_subschema(s1, {'type': 'string'}))
        with self.subTest('not empty array is not only objects'):
            self.assertFalse(is_subschema(s1, {'type': 'object'}))
