            ...
            'X': {
                ...
                'items': {
                    ...
                    'items': [
                        ...
                        {'description': 'condition', 'anyOf': [{'type': 'string'}, {'not': {}}]},
...
def _dataset_operator_cases():
    return [
        pytest.param(op_name, ds_name, id=f"{op_name}-{ds_name}")
        for op_name in TestAIExamples.operator_names
        for ds_name in TestAIExamples.dataset_names
    ]


AI_ENDEAVOR_PARITY_WORK = {
    ("lrOld", "movies"): 12_000,
    ("lrNew", "movies"): 12_000,
    ("project", "movies"): 12_000,
    ("nmf", "housing"): 20_000,
    ("nmf", "movies"): 12_000,
    ("tfidf", "housing"): 20_000,
}


@pytest.mark.parametrize(("op_name", "ds_name"), _dataset_operator_cases())
