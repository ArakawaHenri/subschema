...
from subschema.contracts import ProofResult, UnsupportedDiagnostic
...
class _RegexCharSet:
    ...
    def is_subset_of(self, other: _RegexCharSet) -> bool:
        return not _subtract_codepoint_ranges(
            self._effective_ranges(), other._effective_ranges()
        )

    def is_disjoint_from(self, other: _RegexCharSet) -> bool:
        return not _intersect_codepoint_ranges(
            self._effective_ranges(), other._effective_ranges()
        )
    ...
    def _effective_ranges(self) -> tuple[tuple[int, int], ...]:
        if not self.negated:
            return self.ranges
        return _subtract_codepoint_ranges(((0, _UNICODE_MAX),), self.ranges)


@dataclass(frozen=True)
class _FastRegexModel:
    kind: Literal["exact", "prefix", "repeat"]
    literal: str = ""
    char_set: _RegexCharSet | None = None
    min_length: int = 0
    max_length: int | None = 0
...
class RegexLanguage:
    ...
    def is_subset_of(
        ...
        if self._same_language_as(other) or self._is_empty() or other._is_all():
            return True
        if other._is_empty():
            return self._is_empty()
        fast = _fast_json_regex_subset(self.json_pattern, other.json_pattern)
        if fast is not None:
            return fast
    ...
    def is_disjoint_from(
        ...
        if self._is_empty() or other._is_empty():
            return True
        if self._same_language_as(other):
            return False
        fast = _fast_json_regex_disjoint(self.json_pattern, other.json_pattern)
        if fast is not None:
            return fast
    ...
    def equivalent_to(
        ...
        if self._same_language_as(other):
            return True
        fast = _fast_json_regex_equivalent(self.json_pattern, other.json_pattern)
        if fast is not None:
            return fast
    ...
    def _same_language_as(self, other: RegexLanguage) -> bool:
        if self.pattern is other.pattern:
            return True
        if self.json_pattern is not None and self.json_pattern == other.json_pattern:
            return True
...
@lru_cache(maxsize=_REGEX_CACHE_SIZE)
def _fast_json_regex_subset(lhs: str | None, rhs: str | None) -> bool | None:
    lhs_model = _fast_json_regex_model(lhs)
    rhs_model = _fast_json_regex_model(rhs)
    if lhs_model is None or rhs_model is None:
        return None
    return _fast_regex_model_subset(lhs_model, rhs_model)


@lru_cache(maxsize=_REGEX_CACHE_SIZE)
def _fast_json_regex_disjoint(lhs: str | None, rhs: str | None) -> bool | None:
    lhs_model = _fast_json_regex_model(lhs)
    rhs_model = _fast_json_regex_model(rhs)
    if lhs_model is None or rhs_model is None:
        return None
    return _fast_regex_model_disjoint(lhs_model, rhs_model)


@lru_cache(maxsize=_REGEX_CACHE_SIZE)
def _fast_json_regex_equivalent(lhs: str | None, rhs: str | None) -> bool | None:
    lhs_model = _fast_json_regex_model(lhs)
    rhs_model = _fast_json_regex_model(rhs)
    if lhs_model is None or rhs_model is None:
        return None
    return _fast_regex_model_equivalent(lhs_model, rhs_model)


@lru_cache(maxsize=_REGEX_CACHE_SIZE)
def _fast_json_regex_model(pattern: str | None) -> _FastRegexModel | None:
    if pattern is None:
        return None
    has_start = _has_unescaped_leading_caret(pattern)
    has_end = _has_unescaped_trailing_dollar(pattern)
    if not has_start:
        return None
    body = _strip_supported_anchors(pattern)
    literal = _parse_fast_literal(body)
    if literal is not None:
        if has_end:
            return _FastRegexModel("exact", literal=literal)
        return _FastRegexModel("prefix", literal=literal)
    if not has_end:
        return None
    repeat = _parse_fast_char_set_repeat(body)
    if repeat is None:
        return None
    char_set, min_length, max_length = repeat
    return _FastRegexModel(
        "repeat",
        char_set=char_set,
...
def _fast_regex_model_subset(
    lhs: _FastRegexModel,
    rhs: _FastRegexModel,
) -> bool | None:
    if lhs.kind == "exact":
        return _fast_regex_model_accepts_literal(rhs, lhs.literal)
    if lhs.kind == "prefix":
        if rhs.kind == "prefix":
            return lhs.literal.startswith(rhs.literal)
        if rhs.kind == "exact":
            return False
        return None
    if rhs.kind == "repeat":
        return _repeat_model_subset(lhs, rhs)
    if rhs.kind == "exact":
        singleton = _repeat_model_singleton_literal(lhs)
        return None if singleton is None else singleton == rhs.literal
    if rhs.kind == "prefix":
        singleton = _repeat_model_singleton_literal(lhs)
        return None if singleton is None else singleton.startswith(rhs.literal)
    return None


def _fast_regex_model_disjoint(
    lhs: _FastRegexModel,
    rhs: _FastRegexModel,
) -> bool | None:
    if lhs.kind == "exact":
        accepted = _fast_regex_model_accepts_literal(rhs, lhs.literal)
        return None if accepted is None else not accepted
    if rhs.kind == "exact":
        accepted = _fast_regex_model_accepts_literal(lhs, rhs.literal)
        return None if accepted is None else not accepted
    if lhs.kind == "prefix" and rhs.kind == "prefix":
        return not (
            lhs.literal.startswith(rhs.literal)
            or rhs.literal.startswith(lhs.literal)
        )
    if lhs.kind == "prefix" and rhs.kind == "repeat":
        return _prefix_model_disjoint_from_repeat(lhs, rhs)
    if lhs.kind == "repeat" and rhs.kind == "prefix":
        return _prefix_model_disjoint_from_repeat(rhs, lhs)
    if lhs.kind == "repeat" and rhs.kind == "repeat":
        return _repeat_model_disjoint(lhs, rhs)
    return None


def _fast_regex_model_equivalent(
    lhs: _FastRegexModel,
    rhs: _FastRegexModel,
) -> bool | None:
    if lhs.kind == rhs.kind == "exact":
        return lhs.literal == rhs.literal
    if lhs.kind == rhs.kind == "prefix":
        return lhs.literal == rhs.literal
    if lhs.kind == rhs.kind == "repeat":
        lhs_set = lhs.char_set
        rhs_set = rhs.char_set
        if lhs_set is None or rhs_set is None:
            return None
        return (
            lhs.min_length == rhs.min_length
            and lhs.max_length == rhs.max_length
            and lhs_set.is_subset_of(rhs_set)
            and rhs_set.is_subset_of(lhs_set)
        )
    lhs_subset = _fast_regex_model_subset(lhs, rhs)
    rhs_subset = _fast_regex_model_subset(rhs, lhs)
    if lhs_subset is None or rhs_subset is None:
        return None
    return lhs_subset and rhs_subset


def _fast_regex_model_accepts_literal(
    model: _FastRegexModel,
    value: str,
) -> bool | None:
    if model.kind == "exact":
        return value == model.literal
    if model.kind == "prefix":
        return value.startswith(model.literal)
    char_set = model.char_set
    if char_set is None:
        return None
    return _length_in_repeat_model(model, len(value)) and all(
        char_set.contains_codepoint(ord(char)) for char in value
    )


def _repeat_model_subset(
    lhs: _FastRegexModel,
    rhs: _FastRegexModel,
) -> bool | None:
    lhs_set = lhs.char_set
    ...
    if lhs_set is None or rhs_set is None:
        return None
    if not _length_interval_subset(
        lhs.min_length, lhs.max_length, rhs.min_length, rhs.max_length
    ):
        return False
    if lhs.max_length == 0:
        return True
    return lhs_set.is_subset_of(rhs_set)


def _repeat_model_disjoint(
    lhs: _FastRegexModel,
    rhs: _FastRegexModel,
) -> bool | None:
    lhs_set = lhs.char_set
    ...
    if lhs_set is None or rhs_set is None:
        return None
    shared_min, shared_max = _length_interval_intersection(
        lhs.min_length, lhs.max_length, rhs.min_length, rhs.max_length
    )
    ...
    if shared_min == 0:
        return False
    return lhs_set.is_disjoint_from(rhs_set)


def _prefix_model_disjoint_from_repeat(
    prefix: _FastRegexModel,
    repeat: _FastRegexModel,
) -> bool | None:
    char_set = repeat.char_set
    if char_set is None:
        return None
    if repeat.max_length is not None and repeat.max_length < len(prefix.literal):
        return True
    if any(not char_set.contains_codepoint(ord(char)) for char in prefix.literal):
        return True
    if repeat.max_length == 0:
        return bool(prefix.literal)
    return False


def _repeat_model_singleton_literal(model: _FastRegexModel) -> str | None:
    char_set = model.char_set
    if (
        char_set is None
        or model.max_length is None
        or model.min_length != model.max_length
    ):
        return None
    if model.min_length == 0:
        return ""
    codepoint = char_set.singleton_codepoint()
    return None if codepoint is None else chr(codepoint) * model.min_length


def _length_in_repeat_model(model: _FastRegexModel, length: int) -> bool:
    if length < model.min_length:
        return False
    return model.max_length is None or length <= model.max_length


def _length_interval_subset(
    lhs_min: int,
    lhs_max: int | None,
    rhs_min: int,
    rhs_max: int | None,
    ...
    if rhs_max is None:
        return True
    return lhs_max is not None and lhs_max <= rhs_max


def _length_interval_intersection(
    lhs_min: int,
    lhs_max: int | None,
    rhs_min: int,
    rhs_max: int | None,
) -> tuple[int, int | None] | tuple[None, None]:
    lower = max(lhs_min, rhs_min)
    if lhs_max is None:
        upper = rhs_max
    elif rhs_max is None:
        upper = lhs_max
    else:
        upper = min(lhs_max, rhs_max)
    if upper is not None and lower > upper:
        return None, None
    return lower, upper


def _parse_fast_literal(pattern: str) -> str | None:
    parts: list[str] = []
    index = 0
    while index < len(pattern):
        parsed = _parse_fast_literal_char(pattern, index)
        if parsed is None:
            return None
        char, index = parsed
        parts.append(char)
...
def _parse_fast_literal_char(pattern: str, index: int) -> tuple[str, int] | None:
    ...
    if char == "\\":
        ...
        if _ecma_escape_char_set(escaped) is not None:
            return None
...
def _parse_fast_char_set_repeat(
    pattern: str,
) -> tuple[_RegexCharSet, int, int | None] | None:
    parsed_atom = _parse_fast_char_set_atom(pattern, 0)
    if parsed_atom is None:
        return None
    char_set, index = parsed_atom
    parsed_quantifier = _parse_fast_regex_quantifier_range(pattern, index)
    min_length: int
    max_length: int | None
    if parsed_quantifier is None:
        min_length, max_length = 1, 1
    else:
        min_length, max_length, index = parsed_quantifier
    if index != len(pattern):
        return None
    return char_set, min_length, max_length


def _parse_fast_char_set_atom(
    pattern: str,
    index: int,
    ...
    if char == "\\":
        ...
        if literal_escape is not None:
            ...
            return _RegexCharSet.singleton(literal), next_index
        if index + 1 >= len(pattern):
            return None
        escaped = pattern[index + 1]
        if escaped in _ECMA_CONTROL_ESCAPES:
            return _RegexCharSet.singleton(_ECMA_CONTROL_ESCAPES[escaped]), index + 2
        escape_char_set = _ecma_escape_char_set(escaped)
        if escape_char_set is not None:
            return escape_char_set, index + 2
        return _RegexCharSet.singleton(escaped), index + 2
    if char in ".[]()|*+?{}":
        return None
    return _RegexCharSet.singleton(char), index + 1


def _parse_fast_regex_quantifier_range(
    pattern: str,
    index: int,
    ...
    if upper is not None and lower > upper:
        return None
    return lower, upper, index + match.end()
