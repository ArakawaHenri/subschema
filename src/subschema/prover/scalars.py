...
from subschema.work_protocols import RegexWorkContext, SymbolicContext
...
def type_difference_plan_from_constraints(
    ...
    witness = type_atom_witness(sorted(extra_atoms)[0])
...
def typed_scalar_difference_plan_from_constraints(
    ...
    if extra_atoms:
        if extra_atoms & {"integer", "number"} and lhs_numeric is not None:
            witness = lhs_numeric.witness_not_in(
                NumericConstraint((), accepts_non_numeric=True)
            )
        ...
        return ScalarDifferencePlan.counterexample(
            type_atom_witness(sorted(extra_atoms)[0]),
    ...
    if "string" in lhs_atoms:
        if lhs_string is None and rhs_string is None:
            ...
            string_subset = lhs_string.pattern.is_subset_of(rhs_string.pattern)
            if isinstance(string_subset, ProofResult):
                return string_subset
            if not string_subset:
                string_witness = lhs_string.witness_not_in(rhs_string)
...
def numeric_difference_plan_from_constraints(
    ...
    *,
    ...
    nonnumeric_coverage_complete = _nonnumeric_coverage_is_complete(
        lhs_shape,
        rhs_shape,
        lhs_type,
        rhs_type,
    ...
    if subset is True:
        if not nonnumeric_coverage_complete:
            return ScalarDifferencePlan.unsupported(
    ...
    if context is not None:
        ...
        if not isinstance(symbolic, ProofResult) and symbolic.status != "unsupported":
            if symbolic.status == "proved_true" and not nonnumeric_coverage_complete:
                return ScalarDifferencePlan.unsupported(
                    "SAT numeric fragment cannot prove non-numeric type coverage"
                )
            return symbolic
...
def _nonnumeric_coverage_is_complete(
    lhs_shape: NumericConstraint,
    rhs_shape: NumericConstraint,
    lhs_type: TypeConstraint | None,
    rhs_type: TypeConstraint | None,
) -> bool:
    if not lhs_shape.accepts_non_numeric:
        return True
    if rhs_shape.exact and rhs_shape.accepts_non_numeric:
        return True
    if lhs_type is None or rhs_type is None:
        return False
...
def string_length_difference_plan_from_constraints(
    ...
    if not rhs_shape.exact:
        return ScalarDifferencePlan.unsupported(
            "SAT string-length fragment requires exact right string semantics"
        )
    if lhs_shape.is_subset_of(rhs_shape):
        return ScalarDifferencePlan.true()
...
def string_language_difference_plan_from_constraints(
    ...
    *,
    ...
    if not rhs_shape.exact:
        return ScalarDifferencePlan.unsupported(
            "SAT string-language fragment requires exact right string semantics"
        )
    if lhs_shape.is_subset_of(rhs_shape):
        return ScalarDifferencePlan.true()
...
def _finite_rhs_numeric_witness(
    ...
    witness = lhs_numeric.witness_not_in(rhs_numeric)
...
def _numeric_shape_for_finite_values(values: tuple[Any, ...]) -> NumericConstraint:
    atoms = []
    for value in values:
    ...
    return NumericConstraint(tuple(atoms), accepts_non_numeric=False)
