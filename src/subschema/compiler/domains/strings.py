...
def string_schema_has_finite_language_for_values(schema: Any) -> bool:
    if not isinstance(schema, dict):
        return False
    if not _type_keyword_is_exactly_string(schema.get("type")):
        return False
...
def _type_keyword_is_exactly_string(type_keyword: Any) -> bool:
    type_shape = type_shape_for_type_keyword(type_keyword)
    return type_shape is not None and type_shape.atoms == {"string"}
