...
from subschema.values import dedupe, json_semantic_key
...
def finite_values_for_type_schema(schema: Any) -> list[Any] | None:
    if not isinstance(schema, dict):
        return None
    return _finite_values_for_type_keyword(schema.get("type"))
...
def schema_covers_type_atom(schema: Any, atom: str, depth: int = 0) -> bool:
    ...
    if schema is False or not isinstance(schema, dict):
        return False
    if _has_direct_reference_keyword(schema):
        return False
...
def _has_direct_reference_keyword(schema: dict[str, Any]) -> bool:
    return bool({"$ref", "$recursiveRef", "$dynamicRef"} & schema.keys())
