...
from subschema.kernel.composition import proof_is_inconclusive
...
class ObjectPropertyCountDomainTactic:
    ...
    def is_subschema(self, lhs: Any, rhs: Any) -> ProofResult:
        if contains_reference_keyword(
            ...
            return ProofResult.unsupported(
                "object property-count proof is deferred for static recursive "
                "references"
            )
        ...
        if lhs_shape is None or rhs_shape is None:
            return ProofResult.unsupported(
                "schema is outside the exact object property-count fragment"
            )
        if lhs_shape.accepts_non_object:
            return ProofResult.unsupported("left schema is not object-only")
        ...
        if witness is None:
            return ProofResult.unsupported(
                "object property-count counterexample could not be constructed"
            )
        ...
        return ProofResult.unsupported(
            "object property-count counterexample was rejected by concrete validation"
        )


class ObjectPresenceDomainTactic:
    def __init__(self, context: ProofContext):
        self.context = context
        self.dialect = self.context.dialect
    ...
    def is_subschema(self, lhs: Any, rhs: Any) -> ProofResult:
        if contains_reference_keyword(
            ...
            return ProofResult.unsupported(
                "object presence proof is deferred for static recursive references"
            )

        names = _object_presence_names_for_schemas(lhs, rhs)
        if names is None:
            return ProofResult.unsupported(
        ...
        for atom in sorted(JSON_TYPE_ATOMS - {"object"}):
            lhs_accepts = _object_presence_schema_accepts(lhs, atom, frozenset())
            rhs_accepts = _object_presence_schema_accepts(rhs, atom, frozenset())
            ...
            if lhs_accepts and not rhs_accepts:
                ...
                if backend.validates_difference(lhs, rhs, witness):
                    return ProofResult.false(witness)
                return ProofResult.unsupported(
                    "object presence non-object counterexample was rejected by "
                    "concrete validation"
                )

        symbolic = _object_presence_symbolic_difference(lhs, rhs, names, self.context)
        if isinstance(symbolic, ProofResult):
            return symbolic
        if symbolic is None:
            return ProofResult.unsupported(
                "schema is outside the exact object presence fragment"
        ...
        if symbolic is not False:
            ...
            if backend.validates_difference(lhs, rhs, witness):
                return ProofResult.false(witness)
            return ProofResult.unsupported(
                "object presence counterexample was rejected by concrete validation"
            )

        return ProofResult.true()


class ObjectStructureDomainTactic:
    def __init__(self, context: ProofContext):
        self.context = context
        self.dialect = self.context.dialect
    ...
    def is_subschema(self, lhs: Any, rhs: Any) -> ProofResult:
        if contains_reference_keyword(
            ...
            return ProofResult.unsupported(
                "object structure proof is deferred for static recursive references"
            )

        names = _object_structure_names_for_schemas(lhs, rhs)
        if names is None:
            return ProofResult.unsupported(
        ...
        for atom in sorted(JSON_TYPE_ATOMS - {"object"}):
            lhs_accepts = _object_structure_accepts_non_object(lhs, atom)
            rhs_accepts = _object_structure_accepts_non_object(rhs, atom)
            if lhs_accepts is None or rhs_accepts is None:
                return ProofResult.unsupported(
                    "schema is outside the exact object structure fragment"
                )
            if lhs_accepts and not rhs_accepts:
                ...
                if backend.validates_difference(lhs, rhs, witness):
                    return ProofResult.false(witness)
                return ProofResult.unsupported(
                    "object structure non-object counterexample was rejected by "
                    "concrete validation"
                )

        symbolic = _object_structure_symbolic_difference(lhs, rhs, names, self.context)
        if isinstance(symbolic, ProofResult):
            return symbolic
        if symbolic is None:
            return ProofResult.unsupported(
        ...
        if symbolic is not False:
            ...
            if backend.validates_difference(lhs, rhs, witness):
                return ProofResult.false(witness)
            return ProofResult.unsupported(
                "object structure counterexample was rejected by concrete validation"
            )

        return ProofResult.true()


class ObjectClosedPropertiesDomainTactic:
    def __init__(self, context: ProofContext):
        self.context = context
        self.dialect = self.context.dialect

    def is_subschema(self, lhs: Any, rhs: Any) -> ProofResult:
        if schema_is_false(lhs) or schema_is_true(rhs) or schemas_equal(lhs, rhs):
            return ProofResult.true()
        if contains_reference_keyword(
            lhs, {"$ref", "$recursiveRef"}
        ) or contains_reference_keyword(rhs, {"$ref", "$recursiveRef"}):
            return ProofResult.unsupported(
                "closed object properties proof is deferred for static "
        ...
        if lhs_shape is None or rhs_shape is None:
            return ProofResult.unsupported(
                "schema is outside the exact closed object properties fragment"
            )
        if not lhs_shape.has_finite_keyspace:
            return ProofResult.unsupported("left closed object keyspace is not finite")
        if lhs_shape.accepts_non_object:
            return ProofResult.unsupported("left schema is not object-only")
        if not lhs_shape.object_is_inhabited():
            return ProofResult.true()
        if not rhs_shape.accepts_object:
            witness = lhs_shape.object_witness(self.dialect)
            if witness is None:
                return ProofResult.unsupported(
                    "closed object counterexample could not be constructed"
                )
            return ProofResult.false(witness)
        if not lhs_shape.keyspace_satisfies(rhs_shape):
            witness = lhs_shape.keyspace_witness_not_in(rhs_shape, self.dialect)
            if witness is None:
                return ProofResult.unsupported(
                    "closed object keyspace counterexample could not be constructed"
                )
            return ProofResult.false(witness)

        for name in sorted(lhs_shape.allowed_names):
            lhs_property_schema = lhs_shape.property_schema_for(name)
            rhs_property_schema = rhs_shape.property_schema_for(name)
            if schema_is_true(rhs_property_schema):
                continue
            if not _closed_property_value_schema_is_supported(lhs_property_schema):
                return ProofResult.unsupported(
                    "left property value schema is outside the scalar closed "
                    "object fragment"
                )
            if not _closed_property_value_schema_is_supported(rhs_property_schema):
                return ProofResult.unsupported(
                    "right property value schema is outside the scalar closed "
                    "object fragment"
                )

            proof = self.context.subproof(lhs_property_schema, rhs_property_schema)
            if proof_is_inconclusive(proof):
                return ProofResult.unsupported(
                    proof.reason or "property value proof is unsupported", proof.error
                )
            if proof.status == "proved_false":
                value = proof.witness
                if value is None:
                    value = _constructive_value_satisfying_not_schema(
                        lhs_property_schema, rhs_property_schema, self.dialect
                    )
                witness = lhs_shape.object_witness(self.dialect, override=(name, value))
                if witness is None:
                    return ProofResult.unsupported(
                        "closed object value counterexample could not be constructed"
                    )
                ...
                if backend.validates_difference(lhs, rhs, witness):
                    return ProofResult.false(witness)
                return ProofResult.unsupported(
                    "closed object value counterexample was rejected by concrete "
                    "validation"
                )
...
class ObjectPropertyValuesDomainTactic:
    def __init__(self, context: ProofContext):
        self.context = context
        self.dialect = self.context.dialect
    ...
    def is_subschema(self, lhs: Any, rhs: Any) -> ProofResult:
        ...
        if contains_reference_keyword(
            ...
            return ProofResult.unsupported(
                "object property-values proof is deferred for static "
                "recursive references"
            )

        lhs_shape = object_property_values_shape_for_schema(lhs)
        rhs_shape = object_property_values_shape_for_schema(rhs)
        if lhs_shape is None or rhs_shape is None:
            return ProofResult.unsupported(
                "schema is outside the exact object property-values fragment"
            )

        backend = validation_backend_for(self.dialect)

        if lhs_shape.accepts_non_object and not rhs_shape.accepts_non_object:
            witness = witness_for_type_atom(
                next(iter(sorted(JSON_TYPE_ATOMS - {"object"})))
            )
            if backend.validates_difference(lhs, rhs, witness):
                return ProofResult.false(witness)
            return ProofResult.unsupported(
                "object property-values non-object counterexample was rejected"
            )
        ...
        if not rhs_shape.accepts_object:
            ...
            if witness is None:
                return ProofResult.unsupported(
                    "object property-values counterexample could not be constructed"
                )
            if backend.validates_difference(lhs, rhs, witness):
                return ProofResult.false(witness)
            return ProofResult.unsupported(
                "object property-values counterexample was rejected"
            )

        for name in sorted(rhs_shape.property_names):
            lhs_property_schema = lhs_shape.property_schema_for(name)
            rhs_property_schema = rhs_shape.property_schema_for(name)
            if schema_is_true(rhs_property_schema):
                continue
            if not _closed_property_value_schema_is_supported(lhs_property_schema):
                return ProofResult.unsupported(
                    "left property value schema is outside the scalar object fragment"
                )
            if not _closed_property_value_schema_is_supported(rhs_property_schema):
                return ProofResult.unsupported(
                    "right property value schema is outside the scalar object fragment"
                )
...
class ObjectPropertyNamesDomainTactic:
    def __init__(self, context: ProofContext):
        self.context = context
        self.dialect = self.context.dialect
    ...
    def is_subschema(self, lhs: Any, rhs: Any) -> ProofResult:
        if contains_reference_keyword(
            ...
            return ProofResult.unsupported(
                "object propertyNames proof is deferred for static recursive references"
            )
        if _object_property_names_schema_has_value_constraints(rhs):
            return ProofResult.unsupported(
                "right schema has property value constraints outside keyspace proof"
            )

        lhs_shape = object_property_names_shape_for_schema(lhs)
        rhs_shape = object_property_names_shape_for_schema(rhs)
        if lhs_shape is None or rhs_shape is None:
            return ProofResult.unsupported(
                "schema is outside the exact object propertyNames fragment"
            )
        if lhs_shape.is_subset_of(rhs_shape):
            return ProofResult.true()
        ...
        if witness is None:
            return ProofResult.unsupported(
                "object propertyNames counterexample could not be constructed"
            )

        backend = validation_backend_for(self.dialect)
        if backend.validates_difference(lhs, rhs, witness):
            return ProofResult.false(witness)
        repaired = _repair_object_witness_for_schema(lhs, witness, self.dialect)
        if repaired is not None and backend.validates_difference(lhs, rhs, repaired):
            return ProofResult.false(repaired)
        return ProofResult.unsupported(
            "object propertyNames counterexample was rejected by concrete validation"
        )
